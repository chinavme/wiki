// ==UserScript==
// @author      Shyangs
// @name        Meihua Chinese Converter for userChromeJS
// @description 梅花繁簡轉換器
// @namespace   https://github.com/shyangs/Meihua-Chinese-Converter
// @include     chrome://browser/content/browser.xul
// @version     1.0a7
// @grant       none
// @charset		UTF-8
// @icon        http://www.gravatar.com/avatar/b4067537364e89cce0d6f91e193420d0
// @license     GPLv3; http://opensource.org/licenses/gpl-3.0.html
// ==/UserScript==

/* USAGE:
 * window.MeihuaCC.forceTrans={aTables: ['梅花通用單字', '梅花通用詞彙']}; window.MeihuaCC.onPageLoad({originalTarget:document})
 *
 * EDIT:
 * Convert to ES5 with Babal.js.
 * Line 'if (MeihuaCC.forceTrans) return MeihuaCC.forceTrans;' Inserted.
 */

'use strict';

var _typeof = typeof Symbol === "function" && typeof Symbol.iterator === "symbol" ? function (obj) { return typeof obj; } : function (obj) { return obj && typeof Symbol === "function" && obj.constructor === Symbol ? "symbol" : typeof obj; };

(function () {
	var isObject = function isObject(obj) {
		return obj && (typeof obj === 'undefined' ? 'undefined' : _typeof(obj)) === 'object';
	};

	if (Object.assign) return;
	Object.defineProperty(Object, 'assign', {
		value: function value(target, source) {
			var s, i, props;
			if (!isObject(target)) {
				throw new TypeError('target must be an object');
			}
			for (s = 1; s < arguments.length; ++s) {
				source = arguments[s];
				if (!isObject(source)) {
					throw new TypeError('source ' + s + ' must be an object');
				}
				props = Object.keys(Object(source));
				for (i = 0; i < props.length; ++i) {
					target[props[i]] = source[props[i]];
				}
			}
			return target;
		},
		enumerable: false
	});
})();
(function (window) {

	var MeihuaCC = function () {
		var doc = document,
		    oTables = {},
		    oCacheTable = {},
		    userOpt = {
			bAlt: true,
			bTitle: true,
			aURLs: [{ pattern: '\\.tw/', rule: 'exclude' }, { pattern: 'https?://tw\\.', rule: 'exclude' }, { pattern: '[/.]big5[/.]', rule: 'exclude' }, { pattern: '\\.jp/', rule: 'exclude' }, { pattern: 'https?://jp\\.', rule: 'exclude' }, { pattern: 'wikipedia\\.org/', rule: 'exclude' }, { pattern: '[-_=./]cn(?:[./]|$)', rule: 'include', aTables: ['梅花通用單字', '梅花通用詞彙'] }, { pattern: '[-_=./]gbk?(?:[./]|$)' }, { pattern: '123yq\\.com/' }, { pattern: '\\.163\\.com/' }, { pattern: '\\.17k\\.com/' }, { pattern: '360doc\\.com/' }, { pattern: 'alipay\\.com/' }, { pattern: '\\.b5m\\.com/' }, { pattern: 'baidu\\.com/' }, { pattern: 'china\\.com/' }, { pattern: 'douban\\.com/' }, { pattern: '\\.dm5\\.com/' }, { pattern: 'hao123\\.com/' }, { pattern: 'hongxiu\\.com/' }, { pattern: 'ifeng\\.net/' }, { pattern: 'jjwxc\\.net/' }, { pattern: 'qdmm\\.com/' }, { pattern: 'qidian\\.com/' }, { pattern: '\\.qq\\.com/' }, { pattern: 'readnovel\\.com/' }, { pattern: 'sfacg\\.com/' }, { pattern: 'sina\\.com/' }, { pattern: '\\.so\\.com/' }, { pattern: 'sogou\\.com/' }, { pattern: 'sohu\\.com/' }, { pattern: 'soso\\.com/' }, { pattern: 'taobao\\.com/' }, { pattern: 'thethirdmedia\\.com/' }, { pattern: 'tudou\\.com/' }, { pattern: 'weibo\\.com/' }, { pattern: 'xinhuanet\\.com/' }, { pattern: 'youku\\.com/' }, { pattern: 'zongheng\\.com/' }]
		},
		    observeOpt = {
			childList: true,
			subtree: true
		};
		var addTable = function addTable(table) {
			oTables[table.name] = table;
		},
		    setTable = function setTable(oURL) {
			var table = { mappings: {}, maxLen: 0, id: '' },
			    aTables = oURL.aTables;
			if ('undefined' === typeof aTables || !Array.isArray(aTables) || aTables.length === 0) {
				aTables = ['梅花通用單字', '梅花通用詞彙'];
			}

			table.id = aTables.length.toString();
			aTables.forEach(function (tableName) {
				var version = oTables[tableName].version || Date.now();
				table.id += ',' + tableName + version;
			});
			if ('undefined' !== typeof oCacheTable[table.id]) return oCacheTable[table.id];

			aTables.forEach(function (tableName) {
				var item = oTables[tableName];
				Object.assign(table.mappings, item.mappings);
				table.maxLen = Math.max(table.maxLen, item.maxLen);
			});
			oCacheTable[table.id] = table;
			return table;
		},
		    observerCallback = function observerCallback(mutations, self) {
			mutations.forEach(function (mutation) {
				var _iteratorNormalCompletion = true;
				var _didIteratorError = false;
				var _iteratorError = undefined;

				try {
					for (var _iterator = mutation.addedNodes[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
						var node = _step.value;

						switch (node.nodeType) {
							case 1:
								// ELEMENT_NODE
								transPage(node, false, self.table);
								break;
							case 3:
								// TEXT_NODE
								node.nodeValue = convert(node.nodeValue, self.table);
								break;
						}
					}
				} catch (err) {
					_didIteratorError = true;
					_iteratorError = err;
				} finally {
					try {
						if (!_iteratorNormalCompletion && _iterator.return) {
							_iterator.return();
						}
					} finally {
						if (_didIteratorError) {
							throw _iteratorError;
						}
					}
				}
			});
		},
		    convert = function convert(str, table) {
			var leng = Math.min(table.maxLen, str.length);
			var mappings = table.mappings;
			var txt = '';
			for (var idx = 0, strLen = str.length; idx < strLen;) {
				var bHit = false;
				for (var j = leng; j > 0; j--) {
					var ss = str.substr(idx, j);
					if ('undefined' !== typeof mappings[ss]) {
						txt += mappings[ss];
						idx += j;
						bHit = true;
						break;
					}
				}

				if (!bHit) {
					txt += str.substr(idx, 1);
					idx++;
				}
			}
			if (txt !== '') str = txt;
			return str;
		},
		    walkStep = function walkStep(walker, attr, startTime, table) {
			var node = walker.nextNode();
			if (!node) return;
			if (Date.now() - startTime < 50) {
				node[attr] = convert('nodeValue' === attr ? node.nodeValue : node.getAttribute(attr), table);
				walkStep(walker, attr, startTime, table);
			} else {
				setTimeout(function () {
					walkStep(walker, attr, Date.now(), table);
				}, 1);
			}
		},
		    treeWalker = function treeWalker(root, whatToShow, attr, table) {
			var filter = 'nodeValue' === attr ? {
				acceptNode: function acceptNode(node) {
					switch (node.parentNode.nodeName.toUpperCase()) {
						case 'SCRIPT':
						case 'STYLE':
							return NodeFilter.FILTER_REJECT;
					}
					return NodeFilter.FILTER_ACCEPT;
				}
			} : {
				acceptNode: function acceptNode(node) {
					return node.hasAttribute(attr) ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_SKIP;
				}
			},
			    walker = document.createTreeWalker(root, whatToShow, filter);
			walkStep(walker, attr, Date.now(), table);
		},
		    transPage = function transPage() {
			var elmt = arguments.length <= 0 || arguments[0] === undefined ? doc : arguments[0];
			var bObs = arguments.length <= 1 || arguments[1] === undefined ? true : arguments[1];
			var table = arguments.length <= 2 || arguments[2] === undefined ? setTable({}) : arguments[2];

			if (bObs) {
				var observer = new MutationObserver(observerCallback);
				observer.table = table;
				observer.observe(doc, observeOpt);
			}

			treeWalker(elmt, NodeFilter.SHOW_TEXT, 'nodeValue', table);
			if (userOpt.bTitle) treeWalker(elmt, NodeFilter.SHOW_ELEMENT, 'title', table);
			if (userOpt.bAlt) treeWalker(elmt, NodeFilter.SHOW_ELEMENT, 'alt', table);
		},
		    applyURL = function applyURL(href) {
			var aURLs = userOpt.aURLs;
			if (MeihuaCC.forceTrans) return MeihuaCC.forceTrans;
			for (var i = 0, len = aURLs.length; i < len; i++) {
				var oURL = aURLs[i],
				    pattern = oURL.pattern,
				    rule = oURL.rule,
				    regexp = new RegExp(pattern, 'i');
				if (!regexp.test(href)) continue;
				if (rule !== 'exclude') {
					console.log('MeihuaCC: 網址「 ' + href + ' 」符合包含規則「 ' + pattern + ' 」，繁簡轉換開始...');
					return oURL;
				} else {
					console.log('MeihuaCC: 網址「 ' + href + ' 」符合排除規則「 ' + pattern + ' 」，不進行繁簡轉換。');
					return false;
				}
			}
			console.log('MeihuaCC: 網址「 ' + href + ' 」不符合所有規則，不進行繁簡轉換。');
			return false;
		},
		    onPageLoad = function onPageLoad(aEvent) {
			doc = aEvent.originalTarget; // doc is document that triggered "onload" event
			// do something with the loaded page.
			var oURL = void 0;
			if (!(oURL = applyURL(doc.location.href))) return;
			var table = setTable(oURL);
			var startTime = Date.now();
			MeihuaCC.transPage(doc, true, table);
			console.log('MeihuaCC: 轉換耗時 ' + (Date.now() - startTime) + ' ms.');
		};

		return {
			addTable: addTable,
			onPageLoad: onPageLoad,
			transPage: transPage,
			userOpt: userOpt
		};
	}();

	MeihuaCC.addTable({
		name: '梅花通用單字',
		maxLen: 1,
		version: 140712,
		mappings: {
			"捂": "摀",
			"胄": "冑",
			"丑": "醜",
			"咸": "鹹",
			"眯": "瞇",
			"赞": "讚",
			"脏": "髒",
			"㖞": "喎",
			"㧟": "擓",
			"㳠": "澾",
			"䁖": "瞜",
			"䌷": "紬",
			"䜣": "訢",
			"䞍": "䝼",
			"䥺": "釾",
			"䥽": "鏺",
			"䦃": "鐯",
			"䦅": "鐥",
			"䲟": "鮣",
			"䲡": "鰌",
			"䲢": "鰧",
			"䲣": "䱷",
			"䴓": "鳾",
			"䴔": "鵁",
			"䴕": "鴷",
			"䴖": "鶄",
			"䴗": "鶪",
			"䶮": "龑",
			"万": "萬",
			"与": "與",
			"专": "專",
			"业": "業",
			"丛": "叢",
			"东": "東",
			"丝": "絲",
			"丢": "丟",
			"两": "兩",
			"严": "嚴",
			"丧": "喪",
			"个": "個",
			"丰": "豐",
			"临": "臨",
			"丽": "麗",
			"举": "舉",
			"义": "義",
			"乌": "烏",
			"乐": "樂",
			"乔": "喬",
			"习": "習",
			"乡": "鄉",
			"书": "書",
			"买": "買",
			"乱": "亂",
			"争": "爭",
			"于": "於",
			"亏": "虧",
			"亚": "亞",
			"亩": "畝",
			"亲": "親",
			"亵": "褻",
			"亿": "億",
			"仅": "僅",
			"从": "從",
			"仑": "侖",
			"仓": "倉",
			"仪": "儀",
			"们": "們",
			"价": "價",
			"众": "眾",
			"优": "優",
			"会": "會",
			"伛": "傴",
			"伞": "傘",
			"伟": "偉",
			"传": "傳",
			"伤": "傷",
			"伥": "倀",
			"伦": "倫",
			"伧": "傖",
			"伪": "偽",
			"伫": "佇",
			"佥": "僉",
			"侠": "俠",
			"侣": "侶",
			"侥": "僥",
			"侦": "偵",
			"侧": "側",
			"侨": "僑",
			"侩": "儈",
			"侪": "儕",
			"侬": "儂",
			"俣": "俁",
			"俦": "儔",
			"俨": "儼",
			"俩": "倆",
			"俪": "儷",
			"俭": "儉",
			"债": "債",
			"倾": "傾",
			"偬": "傯",
			"偻": "僂",
			"偾": "僨",
			"偿": "償",
			"傥": "儻",
			"傧": "儐",
			"储": "儲",
			"傩": "儺",
			"儿": "兒",
			"兑": "兌",
			"兖": "兗",
			"党": "黨",
			"兰": "蘭",
			"关": "關",
			"兴": "興",
			"兹": "茲",
			"养": "養",
			"兽": "獸",
			"冁": "囅",
			"内": "內",
			"冈": "岡",
			"册": "冊",
			"写": "寫",
			"军": "軍",
			"农": "農",
			"冯": "馮",
			"冲": "沖",
			"决": "決",
			"况": "況",
			"冻": "凍",
			"凉": "涼",
			"减": "減",
			"凑": "湊",
			"凛": "凜",
			"几": "幾",
			"凤": "鳳",
			"凫": "鳧",
			"凭": "憑",
			"凯": "凱",
			"击": "擊",
			"凿": "鑿",
			"刍": "芻",
			"刘": "劉",
			"则": "則",
			"刚": "剛",
			"创": "創",
			"删": "刪",
			"别": "別",
			"刬": "剗",
			"刭": "剄",
			"刹": "剎",
			"刽": "劊",
			"刿": "劌",
			"剀": "剴",
			"剂": "劑",
			"剐": "剮",
			"剑": "劍",
			"剥": "剝",
			"剧": "劇",
			"劝": "勸",
			"办": "辦",
			"务": "務",
			"劢": "勱",
			"动": "動",
			"励": "勵",
			"劲": "勁",
			"劳": "勞",
			"势": "勢",
			"勚": "勩",
			"匀": "勻",
			"匦": "匭",
			"匮": "匱",
			"区": "區",
			"医": "醫",
			"华": "華",
			"协": "協",
			"单": "單",
			"卖": "賣",
			"卢": "盧",
			"卤": "鹵",
			"卫": "衛",
			"却": "卻",
			"卺": "巹",
			"厂": "廠",
			"厅": "廳",
			"历": "歷",
			"厉": "厲",
			"压": "壓",
			"厌": "厭",
			"厍": "厙",
			"厕": "廁",
			"厢": "廂",
			"厣": "厴",
			"厦": "廈",
			"厨": "廚",
			"厩": "廄",
			"厮": "廝",
			"县": "縣",
			"双": "雙",
			"发": "發",
			"变": "變",
			"叙": "敘",
			"叠": "疊",
			"叶": "葉",
			"号": "號",
			"叹": "嘆",
			"叽": "嘰",
			"后": "後",
			"吓": "嚇",
			"吕": "呂",
			"吗": "嗎",
			"吨": "噸",
			"听": "聽",
			"启": "啟",
			"吴": "吳",
			"呐": "吶",
			"呒": "嘸",
			"呓": "囈",
			"呕": "嘔",
			"呖": "嚦",
			"呗": "唄",
			"员": "員",
			"呙": "咼",
			"呛": "嗆",
			"呜": "嗚",
			"咏": "詠",
			"咙": "嚨",
			"咛": "嚀",
			"咝": "噝",
			"响": "響",
			"哑": "啞",
			"哒": "噠",
			"哓": "嘵",
			"哔": "嗶",
			"哕": "噦",
			"哗": "嘩",
			"哙": "噲",
			"哜": "嚌",
			"哝": "噥",
			"哟": "喲",
			"唛": "嘜",
			"唝": "嗊",
			"唠": "嘮",
			"唡": "啢",
			"唢": "嗩",
			"唤": "喚",
			"啧": "嘖",
			"啬": "嗇",
			"啭": "囀",
			"啸": "嘯",
			"喷": "噴",
			"喽": "嘍",
			"喾": "嚳",
			"嗫": "囁",
			"嗳": "噯",
			"嘘": "噓",
			"嘤": "嚶",
			"嘱": "囑",
			"噜": "嚕",
			"嚣": "囂",
			"团": "團",
			"园": "園",
			"囱": "囪",
			"围": "圍",
			"囵": "圇",
			"图": "圖",
			"圆": "圓",
			"圣": "聖",
			"圹": "壙",
			"场": "場",
			"坏": "壞",
			"块": "塊",
			"坚": "堅",
			"坛": "壇",
			"坜": "壢",
			"坝": "壩",
			"坞": "塢",
			"坟": "墳",
			"坠": "墜",
			"垆": "壚",
			"垒": "壘",
			"垦": "墾",
			"垩": "堊",
			"垫": "墊",
			"垭": "埡",
			"垲": "塏",
			"垴": "堖",
			"埘": "塒",
			"埚": "堝",
			"堑": "塹",
			"堕": "墮",
			"墙": "牆",
			"壮": "壯",
			"声": "聲",
			"壳": "殼",
			"壶": "壺",
			"处": "處",
			"备": "備",
			"复": "復",
			"够": "夠",
			"头": "頭",
			"夸": "誇",
			"夹": "夾",
			"夺": "奪",
			"奁": "奩",
			"奂": "奐",
			"奋": "奮",
			"奥": "奧",
			"妆": "妝",
			"妇": "婦",
			"妈": "媽",
			"妩": "嫵",
			"妪": "嫗",
			"妫": "媯",
			"姗": "姍",
			"娄": "婁",
			"娅": "婭",
			"娆": "嬈",
			"娇": "嬌",
			"娈": "孌",
			"娱": "娛",
			"娲": "媧",
			"娴": "嫻",
			"婳": "嫿",
			"婴": "嬰",
			"婵": "嬋",
			"婶": "嬸",
			"媪": "媼",
			"嫒": "嬡",
			"嫔": "嬪",
			"嫱": "嬙",
			"嬷": "嬤",
			"孙": "孫",
			"学": "學",
			"孪": "孿",
			"宝": "寶",
			"实": "實",
			"宠": "寵",
			"审": "審",
			"宪": "憲",
			"宫": "宮",
			"宽": "寬",
			"宾": "賓",
			"寝": "寢",
			"对": "對",
			"寻": "尋",
			"导": "導",
			"寿": "壽",
			"将": "將",
			"尔": "爾",
			"尘": "塵",
			"尝": "嘗",
			"尧": "堯",
			"尴": "尷",
			"尸": "屍",
			"尽": "盡",
			"层": "層",
			"屉": "屜",
			"届": "屆",
			"属": "屬",
			"屡": "屢",
			"屦": "屨",
			"屿": "嶼",
			"岁": "歲",
			"岂": "豈",
			"岖": "嶇",
			"岗": "崗",
			"岘": "峴",
			"岚": "嵐",
			"岛": "島",
			"岭": "嶺",
			"岿": "巋",
			"峄": "嶧",
			"峡": "峽",
			"峣": "嶢",
			"峤": "嶠",
			"峥": "崢",
			"峦": "巒",
			"崂": "嶗",
			"崃": "崍",
			"崭": "嶄",
			"嵘": "嶸",
			"嵚": "嶔",
			"嵝": "嶁",
			"巅": "巔",
			"巩": "鞏",
			"巯": "巰",
			"币": "幣",
			"帅": "帥",
			"师": "師",
			"帏": "幃",
			"帐": "帳",
			"帘": "簾",
			"帜": "幟",
			"带": "帶",
			"帧": "幀",
			"帮": "幫",
			"帱": "幬",
			"帻": "幘",
			"帼": "幗",
			"幂": "冪",
			"广": "廣",
			"庆": "慶",
			"庐": "廬",
			"庑": "廡",
			"库": "庫",
			"应": "應",
			"庙": "廟",
			"庞": "龐",
			"废": "廢",
			"廪": "廩",
			"异": "異",
			"弃": "棄",
			"弑": "弒",
			"张": "張",
			"弥": "彌",
			"弪": "弳",
			"弯": "彎",
			"弹": "彈",
			"强": "強",
			"归": "歸",
			"当": "當",
			"录": "錄",
			"彦": "彥",
			"彻": "徹",
			"径": "徑",
			"徕": "徠",
			"忆": "憶",
			"忏": "懺",
			"忧": "憂",
			"忾": "愾",
			"怀": "懷",
			"态": "態",
			"怂": "慫",
			"怃": "憮",
			"怄": "慪",
			"怅": "悵",
			"怆": "愴",
			"怜": "憐",
			"总": "總",
			"怼": "懟",
			"怿": "懌",
			"恋": "戀",
			"恒": "恆",
			"恳": "懇",
			"恶": "惡",
			"恸": "慟",
			"恹": "懨",
			"恺": "愷",
			"恻": "惻",
			"恼": "惱",
			"恽": "惲",
			"悦": "悅",
			"悫": "愨",
			"悬": "懸",
			"悭": "慳",
			"悯": "憫",
			"惊": "驚",
			"惧": "懼",
			"惨": "慘",
			"惩": "懲",
			"惫": "憊",
			"惬": "愜",
			"惭": "慚",
			"惮": "憚",
			"惯": "慣",
			"愠": "慍",
			"愤": "憤",
			"愦": "憒",
			"愿": "願",
			"慑": "懾",
			"懑": "懣",
			"懒": "懶",
			"懔": "懍",
			"戆": "戇",
			"戋": "戔",
			"戏": "戲",
			"戗": "戧",
			"战": "戰",
			"戬": "戩",
			"户": "戶",
			"扑": "撲",
			"执": "執",
			"扩": "擴",
			"扪": "捫",
			"扫": "掃",
			"扬": "揚",
			"扰": "擾",
			"抚": "撫",
			"抛": "拋",
			"抟": "摶",
			"抠": "摳",
			"抡": "掄",
			"抢": "搶",
			"护": "護",
			"报": "報",
			"担": "擔",
			"拟": "擬",
			"拢": "攏",
			"拣": "揀",
			"拥": "擁",
			"拦": "攔",
			"拧": "擰",
			"拨": "撥",
			"择": "擇",
			"挂": "掛",
			"挚": "摯",
			"挛": "攣",
			"挜": "掗",
			"挝": "撾",
			"挞": "撻",
			"挟": "挾",
			"挠": "撓",
			"挡": "擋",
			"挢": "撟",
			"挣": "掙",
			"挤": "擠",
			"挥": "揮",
			"挦": "撏",
			"捞": "撈",
			"损": "損",
			"捡": "撿",
			"换": "換",
			"捣": "搗",
			"据": "據",
			"掳": "擄",
			"掴": "摑",
			"掷": "擲",
			"掸": "撣",
			"掺": "摻",
			"掼": "摜",
			"揽": "攬",
			"揿": "撳",
			"搀": "攙",
			"搁": "擱",
			"搂": "摟",
			"搅": "攪",
			"携": "攜",
			"摄": "攝",
			"摅": "攄",
			"摆": "擺",
			"摇": "搖",
			"摈": "擯",
			"摊": "攤",
			"撄": "攖",
			"撑": "撐",
			"撵": "攆",
			"撷": "擷",
			"撸": "擼",
			"撺": "攛",
			"擞": "擻",
			"攒": "攢",
			"敌": "敵",
			"敛": "斂",
			"数": "數",
			"斋": "齋",
			"斓": "斕",
			"斩": "斬",
			"断": "斷",
			"无": "無",
			"旧": "舊",
			"时": "時",
			"旷": "曠",
			"旸": "暘",
			"昙": "曇",
			"昼": "晝",
			"昽": "曨",
			"显": "顯",
			"晋": "晉",
			"晒": "曬",
			"晓": "曉",
			"晔": "曄",
			"晕": "暈",
			"晖": "暉",
			"暂": "暫",
			"暧": "曖",
			"机": "機",
			"杀": "殺",
			"杂": "雜",
			"权": "權",
			"杆": "桿",
			"条": "條",
			"来": "來",
			"杨": "楊",
			"杩": "榪",
			"杰": "傑",
			"极": "極",
			"构": "構",
			"枞": "樅",
			"枢": "樞",
			"枣": "棗",
			"枥": "櫪",
			"枧": "梘",
			"枨": "棖",
			"枪": "槍",
			"枫": "楓",
			"枭": "梟",
			"柜": "櫃",
			"柠": "檸",
			"柽": "檉",
			"栀": "梔",
			"栅": "柵",
			"标": "標",
			"栈": "棧",
			"栉": "櫛",
			"栊": "櫳",
			"栋": "棟",
			"栌": "櫨",
			"栎": "櫟",
			"栏": "欄",
			"树": "樹",
			"栖": "棲",
			"样": "樣",
			"栾": "欒",
			"桠": "椏",
			"桡": "橈",
			"桢": "楨",
			"档": "檔",
			"桤": "榿",
			"桥": "橋",
			"桦": "樺",
			"桧": "檜",
			"桨": "槳",
			"桩": "樁",
			"梦": "夢",
			"检": "檢",
			"椁": "槨",
			"椟": "櫝",
			"椠": "槧",
			"椤": "欏",
			"椭": "橢",
			"楼": "樓",
			"榄": "欖",
			"榇": "櫬",
			"榈": "櫚",
			"榉": "櫸",
			"槚": "檟",
			"槛": "檻",
			"槟": "檳",
			"槠": "櫧",
			"横": "橫",
			"樯": "檣",
			"樱": "櫻",
			"橥": "櫫",
			"橱": "櫥",
			"橹": "櫓",
			"橼": "櫞",
			"檩": "檁",
			"欢": "歡",
			"欤": "歟",
			"欧": "歐",
			"歼": "殲",
			"殁": "歿",
			"殇": "殤",
			"残": "殘",
			"殒": "殞",
			"殓": "殮",
			"殚": "殫",
			"殡": "殯",
			"殴": "毆",
			"毁": "毀",
			"毂": "轂",
			"毕": "畢",
			"毙": "斃",
			"毡": "氈",
			"毵": "毿",
			"氇": "氌",
			"气": "氣",
			"氢": "氫",
			"氩": "氬",
			"氲": "氳",
			"汇": "匯",
			"汉": "漢",
			"汤": "湯",
			"汹": "洶",
			"沟": "溝",
			"没": "沒",
			"沣": "灃",
			"沤": "漚",
			"沥": "瀝",
			"沦": "淪",
			"沧": "滄",
			"沩": "溈",
			"沪": "滬",
			"泞": "濘",
			"泪": "淚",
			"泶": "澩",
			"泷": "瀧",
			"泸": "瀘",
			"泺": "濼",
			"泻": "瀉",
			"泼": "潑",
			"泽": "澤",
			"泾": "涇",
			"洁": "潔",
			"洒": "灑",
			"洼": "窪",
			"浃": "浹",
			"浅": "淺",
			"浆": "漿",
			"浇": "澆",
			"浈": "湞",
			"浊": "濁",
			"测": "測",
			"浍": "澮",
			"济": "濟",
			"浏": "瀏",
			"浐": "滻",
			"浑": "渾",
			"浒": "滸",
			"浓": "濃",
			"浔": "潯",
			"涛": "濤",
			"涝": "澇",
			"涞": "淶",
			"涟": "漣",
			"涠": "潿",
			"涡": "渦",
			"涣": "渙",
			"涤": "滌",
			"润": "潤",
			"涧": "澗",
			"涨": "漲",
			"涩": "澀",
			"渊": "淵",
			"渌": "淥",
			"渍": "漬",
			"渎": "瀆",
			"渐": "漸",
			"渑": "澠",
			"渔": "漁",
			"渖": "瀋",
			"渗": "滲",
			"温": "溫",
			"湾": "灣",
			"湿": "濕",
			"溃": "潰",
			"溅": "濺",
			"溆": "漵",
			"滗": "潷",
			"滚": "滾",
			"滞": "滯",
			"滠": "灄",
			"满": "滿",
			"滢": "瀅",
			"滤": "濾",
			"滥": "濫",
			"滦": "灤",
			"滨": "濱",
			"滩": "灘",
			"滪": "澦",
			"潆": "瀠",
			"潇": "瀟",
			"潋": "瀲",
			"潍": "濰",
			"潜": "潛",
			"潴": "瀦",
			"澜": "瀾",
			"濑": "瀨",
			"濒": "瀕",
			"灏": "灝",
			"灭": "滅",
			"灯": "燈",
			"灵": "靈",
			"灾": "災",
			"灿": "燦",
			"炀": "煬",
			"炉": "爐",
			"炖": "燉",
			"炜": "煒",
			"炝": "熗",
			"点": "點",
			"炼": "煉",
			"炽": "熾",
			"烁": "爍",
			"烂": "爛",
			"烃": "烴",
			"烛": "燭",
			"烟": "煙",
			"烦": "煩",
			"烧": "燒",
			"烨": "燁",
			"烩": "燴",
			"烫": "燙",
			"烬": "燼",
			"热": "熱",
			"焕": "煥",
			"焖": "燜",
			"焘": "燾",
			"爱": "愛",
			"爷": "爺",
			"牍": "牘",
			"牵": "牽",
			"牺": "犧",
			"犊": "犢",
			"状": "狀",
			"犷": "獷",
			"犸": "獁",
			"犹": "猶",
			"狈": "狽",
			"狝": "獮",
			"狞": "獰",
			"独": "獨",
			"狭": "狹",
			"狮": "獅",
			"狯": "獪",
			"狰": "猙",
			"狱": "獄",
			"狲": "猻",
			"猃": "獫",
			"猎": "獵",
			"猕": "獼",
			"猡": "玀",
			"猪": "豬",
			"猫": "貓",
			"猬": "蝟",
			"献": "獻",
			"獭": "獺",
			"玑": "璣",
			"玛": "瑪",
			"玮": "瑋",
			"环": "環",
			"现": "現",
			"玱": "瑲",
			"玺": "璽",
			"珐": "琺",
			"珑": "瓏",
			"珲": "琿",
			"琏": "璉",
			"琐": "瑣",
			"琼": "瓊",
			"瑶": "瑤",
			"瑷": "璦",
			"璎": "瓔",
			"瓒": "瓚",
			"瓯": "甌",
			"电": "電",
			"画": "畫",
			"畅": "暢",
			"畴": "疇",
			"疖": "癤",
			"疗": "療",
			"疟": "瘧",
			"疠": "癘",
			"疡": "瘍",
			"疬": "癧",
			"疮": "瘡",
			"疯": "瘋",
			"疱": "皰",
			"疴": "痾",
			"痈": "癰",
			"痉": "痙",
			"痒": "癢",
			"痖": "瘂",
			"痨": "癆",
			"痪": "瘓",
			"痫": "癇",
			"瘅": "癉",
			"瘗": "瘞",
			"瘪": "癟",
			"瘫": "癱",
			"瘾": "癮",
			"瘿": "癭",
			"癞": "癩",
			"癣": "癬",
			"癫": "癲",
			"皑": "皚",
			"皱": "皺",
			"皲": "皸",
			"盏": "盞",
			"盐": "鹽",
			"监": "監",
			"盖": "蓋",
			"盗": "盜",
			"盘": "盤",
			"眍": "瞘",
			"眦": "眥",
			"眬": "矓",
			"睁": "睜",
			"睐": "睞",
			"睑": "瞼",
			"瞒": "瞞",
			"瞩": "矚",
			"矫": "矯",
			"矶": "磯",
			"矾": "礬",
			"矿": "礦",
			"砀": "碭",
			"码": "碼",
			"砖": "磚",
			"砗": "硨",
			"砚": "硯",
			"砜": "碸",
			"砺": "礪",
			"砻": "礱",
			"砾": "礫",
			"础": "礎",
			"硕": "碩",
			"硖": "硤",
			"硗": "磽",
			"硙": "磑",
			"确": "確",
			"碍": "礙",
			"碛": "磧",
			"碜": "磣",
			"礼": "禮",
			"祎": "禕",
			"祯": "禎",
			"祷": "禱",
			"祸": "禍",
			"禀": "稟",
			"禄": "祿",
			"禅": "禪",
			"离": "離",
			"秃": "禿",
			"秆": "稈",
			"种": "種",
			"积": "積",
			"称": "稱",
			"秽": "穢",
			"税": "稅",
			"稣": "穌",
			"稳": "穩",
			"穑": "穡",
			"穷": "窮",
			"窃": "竊",
			"窍": "竅",
			"窎": "窵",
			"窑": "窯",
			"窜": "竄",
			"窝": "窩",
			"窥": "窺",
			"窦": "竇",
			"窭": "窶",
			"竖": "豎",
			"竞": "競",
			"笃": "篤",
			"笋": "筍",
			"笔": "筆",
			"笕": "筧",
			"笺": "箋",
			"笼": "籠",
			"笾": "籩",
			"筑": "築",
			"筚": "篳",
			"筛": "篩",
			"筝": "箏",
			"筹": "籌",
			"签": "簽",
			"简": "簡",
			"箓": "籙",
			"箦": "簀",
			"箧": "篋",
			"箨": "籜",
			"箩": "籮",
			"箪": "簞",
			"箫": "簫",
			"篑": "簣",
			"篓": "簍",
			"篮": "籃",
			"篱": "籬",
			"簖": "籪",
			"籁": "籟",
			"籴": "糴",
			"类": "類",
			"籼": "秈",
			"粜": "糶",
			"粝": "糲",
			"粤": "粵",
			"粪": "糞",
			"粮": "糧",
			"糁": "糝",
			"糇": "餱",
			"紧": "緊",
			"絷": "縶",
			"纠": "糾",
			"纡": "紆",
			"红": "紅",
			"纣": "紂",
			"纤": "纖",
			"纥": "紇",
			"约": "約",
			"级": "級",
			"纨": "紈",
			"纩": "纊",
			"纪": "紀",
			"纫": "紉",
			"纬": "緯",
			"纭": "紜",
			"纮": "紘",
			"纯": "純",
			"纰": "紕",
			"纱": "紗",
			"纲": "綱",
			"纳": "納",
			"纴": "紝",
			"纵": "縱",
			"纶": "綸",
			"纷": "紛",
			"纸": "紙",
			"纹": "紋",
			"纺": "紡",
			"纼": "紖",
			"纽": "紐",
			"纾": "紓",
			"绀": "紺",
			"绁": "紲",
			"绂": "紱",
			"练": "練",
			"组": "組",
			"绅": "紳",
			"细": "細",
			"织": "織",
			"终": "終",
			"绉": "縐",
			"绊": "絆",
			"绋": "紼",
			"绌": "絀",
			"绍": "紹",
			"绎": "繹",
			"经": "經",
			"绐": "紿",
			"绑": "綁",
			"绒": "絨",
			"结": "結",
			"绔": "絝",
			"绕": "繞",
			"绖": "絰",
			"绗": "絎",
			"绘": "繪",
			"给": "給",
			"绚": "絢",
			"绛": "絳",
			"络": "絡",
			"绝": "絕",
			"绞": "絞",
			"统": "統",
			"绠": "綆",
			"绡": "綃",
			"绢": "絹",
			"绥": "綏",
			"绦": "絛",
			"继": "繼",
			"绨": "綈",
			"绩": "績",
			"绪": "緒",
			"绫": "綾",
			"续": "續",
			"绮": "綺",
			"绯": "緋",
			"绰": "綽",
			"绱": "緔",
			"绲": "緄",
			"绳": "繩",
			"维": "維",
			"绵": "綿",
			"绶": "綬",
			"绸": "綢",
			"绺": "綹",
			"绻": "綣",
			"综": "綜",
			"绽": "綻",
			"绾": "綰",
			"绿": "綠",
			"缀": "綴",
			"缁": "緇",
			"缂": "緙",
			"缃": "緗",
			"缄": "緘",
			"缅": "緬",
			"缆": "纜",
			"缇": "緹",
			"缈": "緲",
			"缉": "緝",
			"缊": "縕",
			"缋": "繢",
			"缌": "緦",
			"缍": "綞",
			"缎": "緞",
			"缏": "緶",
			"缑": "緱",
			"缒": "縋",
			"缓": "緩",
			"缔": "締",
			"缕": "縷",
			"编": "編",
			"缗": "緡",
			"缘": "緣",
			"缙": "縉",
			"缚": "縛",
			"缛": "縟",
			"缜": "縝",
			"缝": "縫",
			"缞": "縗",
			"缟": "縞",
			"缠": "纏",
			"缡": "縭",
			"缢": "縊",
			"缣": "縑",
			"缤": "繽",
			"缥": "縹",
			"缦": "縵",
			"缧": "縲",
			"缨": "纓",
			"缩": "縮",
			"缪": "繆",
			"缫": "繅",
			"缬": "纈",
			"缭": "繚",
			"缮": "繕",
			"缯": "繒",
			"缱": "繾",
			"缲": "繰",
			"缳": "繯",
			"缴": "繳",
			"缵": "纘",
			"罂": "罌",
			"网": "網",
			"罗": "羅",
			"罚": "罰",
			"罢": "罷",
			"罴": "羆",
			"羁": "羈",
			"羟": "羥",
			"羡": "羨",
			"翘": "翹",
			"耢": "耮",
			"耧": "耬",
			"耸": "聳",
			"耻": "恥",
			"聂": "聶",
			"聋": "聾",
			"职": "職",
			"聍": "聹",
			"联": "聯",
			"聩": "聵",
			"聪": "聰",
			"肃": "肅",
			"肠": "腸",
			"肤": "膚",
			"肮": "骯",
			"肾": "腎",
			"肿": "腫",
			"胀": "脹",
			"胁": "脅",
			"胆": "膽",
			"胜": "勝",
			"胧": "朧",
			"胨": "腖",
			"胪": "臚",
			"胫": "脛",
			"胶": "膠",
			"脉": "脈",
			"脍": "膾",
			"脐": "臍",
			"脑": "腦",
			"脓": "膿",
			"脔": "臠",
			"脚": "腳",
			"脱": "脫",
			"脶": "腡",
			"脸": "臉",
			"腊": "臘",
			"腻": "膩",
			"腽": "膃",
			"腾": "騰",
			"膑": "臏",
			"臜": "臢",
			"舆": "輿",
			"舣": "艤",
			"舰": "艦",
			"舱": "艙",
			"舻": "艫",
			"艰": "艱",
			"艺": "藝",
			"节": "節",
			"芈": "羋",
			"芗": "薌",
			"芜": "蕪",
			"芦": "蘆",
			"苁": "蓯",
			"苇": "葦",
			"苈": "藶",
			"苋": "莧",
			"苌": "萇",
			"苍": "蒼",
			"苏": "蘇",
			"苹": "蘋",
			"茎": "莖",
			"茏": "蘢",
			"茑": "蔦",
			"茔": "塋",
			"茕": "煢",
			"茧": "繭",
			"荆": "荊",
			"荐": "薦",
			"荚": "莢",
			"荛": "蕘",
			"荜": "蓽",
			"荞": "蕎",
			"荟": "薈",
			"荠": "薺",
			"荣": "榮",
			"荤": "葷",
			"荥": "滎",
			"荦": "犖",
			"荧": "熒",
			"荨": "蕁",
			"荩": "藎",
			"荪": "蓀",
			"荫": "蔭",
			"荬": "蕒",
			"荭": "葒",
			"荮": "葤",
			"莅": "蒞",
			"莱": "萊",
			"莲": "蓮",
			"莳": "蒔",
			"莴": "萵",
			"莶": "薟",
			"获": "獲",
			"莸": "蕕",
			"莹": "瑩",
			"莺": "鶯",
			"萝": "蘿",
			"萤": "螢",
			"营": "營",
			"萦": "縈",
			"萧": "蕭",
			"萨": "薩",
			"葱": "蔥",
			"蒇": "蕆",
			"蒉": "蕢",
			"蒋": "蔣",
			"蒌": "蔞",
			"蓝": "藍",
			"蓟": "薊",
			"蓠": "蘺",
			"蓣": "蕷",
			"蓥": "鎣",
			"蓦": "驀",
			"蔷": "薔",
			"蔹": "蘞",
			"蔺": "藺",
			"蔼": "藹",
			"蕲": "蘄",
			"蕴": "蘊",
			"薮": "藪",
			"藓": "蘚",
			"虏": "虜",
			"虑": "慮",
			"虚": "虛",
			"虫": "蟲",
			"虬": "虯",
			"虮": "蟣",
			"虽": "雖",
			"虾": "蝦",
			"虿": "蠆",
			"蚀": "蝕",
			"蚁": "蟻",
			"蚂": "螞",
			"蚕": "蠶",
			"蚬": "蜆",
			"蛊": "蠱",
			"蛎": "蠣",
			"蛏": "蟶",
			"蛮": "蠻",
			"蛰": "蟄",
			"蛱": "蛺",
			"蛲": "蟯",
			"蛳": "螄",
			"蛴": "蠐",
			"蜕": "蛻",
			"蜗": "蝸",
			"蜡": "蠟",
			"蝇": "蠅",
			"蝈": "蟈",
			"蝉": "蟬",
			"蝎": "蠍",
			"蝼": "螻",
			"蝾": "蠑",
			"螨": "蟎",
			"蟏": "蠨",
			"衅": "釁",
			"衔": "銜",
			"补": "補",
			"衬": "襯",
			"衮": "袞",
			"袄": "襖",
			"袅": "裊",
			"袜": "襪",
			"袭": "襲",
			"装": "裝",
			"裆": "襠",
			"裢": "褳",
			"裣": "襝",
			"裤": "褲",
			"褛": "褸",
			"褴": "襤",
			"见": "見",
			"观": "觀",
			"觃": "覎",
			"觅": "覓",
			"觇": "覘",
			"览": "覽",
			"觉": "覺",
			"觊": "覬",
			"觋": "覡",
			"觌": "覿",
			"觎": "覦",
			"觏": "覯",
			"觐": "覲",
			"觑": "覷",
			"觞": "觴",
			"触": "觸",
			"觯": "觶",
			"誉": "譽",
			"誊": "謄",
			"计": "計",
			"订": "訂",
			"讣": "訃",
			"认": "認",
			"讥": "譏",
			"讦": "訐",
			"讧": "訌",
			"讨": "討",
			"让": "讓",
			"讪": "訕",
			"讫": "訖",
			"训": "訓",
			"议": "議",
			"讯": "訊",
			"记": "記",
			"讲": "講",
			"讳": "諱",
			"讴": "謳",
			"讵": "詎",
			"讶": "訝",
			"讷": "訥",
			"许": "許",
			"讹": "訛",
			"论": "論",
			"讻": "訩",
			"讼": "訟",
			"讽": "諷",
			"设": "設",
			"访": "訪",
			"诀": "訣",
			"证": "證",
			"诂": "詁",
			"诃": "訶",
			"评": "評",
			"诅": "詛",
			"识": "識",
			"诇": "詗",
			"诈": "詐",
			"诉": "訴",
			"诊": "診",
			"诋": "詆",
			"诌": "謅",
			"词": "詞",
			"诎": "詘",
			"诏": "詔",
			"译": "譯",
			"诒": "詒",
			"诓": "誆",
			"诔": "誄",
			"试": "試",
			"诖": "詿",
			"诗": "詩",
			"诘": "詰",
			"诙": "詼",
			"诚": "誠",
			"诛": "誅",
			"诜": "詵",
			"话": "話",
			"诞": "誕",
			"诟": "詬",
			"诠": "詮",
			"诡": "詭",
			"询": "詢",
			"诣": "詣",
			"诤": "諍",
			"该": "該",
			"详": "詳",
			"诧": "詫",
			"诨": "諢",
			"诩": "詡",
			"诫": "誡",
			"诬": "誣",
			"语": "語",
			"诮": "誚",
			"误": "誤",
			"诰": "誥",
			"诱": "誘",
			"诲": "誨",
			"诳": "誑",
			"说": "說",
			"诵": "誦",
			"诶": "誒",
			"请": "請",
			"诸": "諸",
			"诹": "諏",
			"诺": "諾",
			"读": "讀",
			"诼": "諑",
			"诽": "誹",
			"课": "課",
			"诿": "諉",
			"谀": "諛",
			"谁": "誰",
			"谂": "諗",
			"调": "調",
			"谄": "諂",
			"谅": "諒",
			"谆": "諄",
			"谇": "誶",
			"谈": "談",
			"谊": "誼",
			"谋": "謀",
			"谌": "諶",
			"谍": "諜",
			"谎": "謊",
			"谏": "諫",
			"谐": "諧",
			"谑": "謔",
			"谒": "謁",
			"谓": "謂",
			"谔": "諤",
			"谕": "諭",
			"谖": "諼",
			"谗": "讒",
			"谘": "諮",
			"谙": "諳",
			"谚": "諺",
			"谛": "諦",
			"谜": "謎",
			"谝": "諞",
			"谞": "諝",
			"谟": "謨",
			"谠": "讜",
			"谡": "謖",
			"谢": "謝",
			"谣": "謠",
			"谤": "謗",
			"谥": "謚",
			"谦": "謙",
			"谧": "謐",
			"谨": "謹",
			"谩": "謾",
			"谪": "謫",
			"谫": "譾",
			"谬": "謬",
			"谭": "譚",
			"谮": "譖",
			"谯": "譙",
			"谰": "讕",
			"谱": "譜",
			"谲": "譎",
			"谳": "讞",
			"谴": "譴",
			"谵": "譫",
			"谶": "讖",
			"豮": "豶",
			"贝": "貝",
			"贞": "貞",
			"负": "負",
			"贡": "貢",
			"财": "財",
			"责": "責",
			"贤": "賢",
			"败": "敗",
			"账": "賬",
			"货": "貨",
			"质": "質",
			"贩": "販",
			"贪": "貪",
			"贫": "貧",
			"贬": "貶",
			"购": "購",
			"贮": "貯",
			"贯": "貫",
			"贰": "貳",
			"贱": "賤",
			"贲": "賁",
			"贳": "貰",
			"贴": "貼",
			"贵": "貴",
			"贶": "貺",
			"贷": "貸",
			"贸": "貿",
			"费": "費",
			"贺": "賀",
			"贻": "貽",
			"贼": "賊",
			"贽": "贄",
			"贾": "賈",
			"贿": "賄",
			"赀": "貲",
			"赁": "賃",
			"赂": "賂",
			"资": "資",
			"赅": "賅",
			"赆": "贐",
			"赇": "賕",
			"赈": "賑",
			"赉": "賚",
			"赊": "賒",
			"赋": "賦",
			"赌": "賭",
			"赍": "齎",
			"赎": "贖",
			"赏": "賞",
			"赐": "賜",
			"赒": "賙",
			"赓": "賡",
			"赔": "賠",
			"赕": "賧",
			"赖": "賴",
			"赗": "賵",
			"赘": "贅",
			"赙": "賻",
			"赚": "賺",
			"赛": "賽",
			"赜": "賾",
			"赟": "贇",
			"赠": "贈",
			"赡": "贍",
			"赢": "贏",
			"赣": "贛",
			"赵": "趙",
			"赶": "趕",
			"趋": "趨",
			"趱": "趲",
			"趸": "躉",
			"跃": "躍",
			"跄": "蹌",
			"跞": "躒",
			"践": "踐",
			"跷": "蹺",
			"跸": "蹕",
			"跹": "躚",
			"跻": "躋",
			"踊": "踴",
			"踌": "躊",
			"踪": "蹤",
			"踬": "躓",
			"踯": "躑",
			"蹑": "躡",
			"蹒": "蹣",
			"蹰": "躕",
			"蹿": "躥",
			"躏": "躪",
			"躜": "躦",
			"躯": "軀",
			"车": "車",
			"轧": "軋",
			"轨": "軌",
			"轩": "軒",
			"轫": "軔",
			"转": "轉",
			"轭": "軛",
			"轮": "輪",
			"软": "軟",
			"轰": "轟",
			"轱": "軲",
			"轲": "軻",
			"轳": "轤",
			"轴": "軸",
			"轵": "軹",
			"轶": "軼",
			"轷": "軤",
			"轸": "軫",
			"轹": "轢",
			"轺": "軺",
			"轻": "輕",
			"轼": "軾",
			"载": "載",
			"轾": "輊",
			"轿": "轎",
			"辁": "輇",
			"辂": "輅",
			"较": "較",
			"辄": "輒",
			"辅": "輔",
			"辆": "輛",
			"辇": "輦",
			"辈": "輩",
			"辉": "輝",
			"辊": "輥",
			"辋": "輞",
			"辍": "輟",
			"辎": "輜",
			"辏": "輳",
			"辐": "輻",
			"辑": "輯",
			"输": "輸",
			"辔": "轡",
			"辕": "轅",
			"辖": "轄",
			"辗": "輾",
			"辘": "轆",
			"辙": "轍",
			"辚": "轔",
			"辞": "辭",
			"辩": "辯",
			"辫": "辮",
			"边": "邊",
			"辽": "遼",
			"达": "達",
			"迁": "遷",
			"过": "過",
			"迈": "邁",
			"运": "運",
			"还": "還",
			"这": "這",
			"进": "進",
			"远": "遠",
			"违": "違",
			"连": "連",
			"迟": "遲",
			"迩": "邇",
			"迳": "逕",
			"迹": "跡",
			"适": "適",
			"选": "選",
			"逊": "遜",
			"递": "遞",
			"逦": "邐",
			"逻": "邏",
			"遗": "遺",
			"遥": "遙",
			"邓": "鄧",
			"邝": "鄺",
			"邬": "鄔",
			"邮": "郵",
			"邹": "鄒",
			"邺": "鄴",
			"邻": "鄰",
			"郏": "郟",
			"郐": "鄶",
			"郑": "鄭",
			"郓": "鄆",
			"郦": "酈",
			"郧": "鄖",
			"郸": "鄲",
			"酱": "醬",
			"酽": "釅",
			"酾": "釃",
			"酿": "釀",
			"释": "釋",
			"銮": "鑾",
			"錾": "鏨",
			"钆": "釓",
			"钇": "釔",
			"针": "針",
			"钉": "釘",
			"钊": "釗",
			"钋": "釙",
			"钌": "釕",
			"钍": "釷",
			"钎": "釺",
			"钏": "釧",
			"钐": "釤",
			"钒": "釩",
			"钓": "釣",
			"钔": "鍆",
			"钕": "釹",
			"钖": "鍚",
			"钗": "釵",
			"钘": "鈃",
			"钙": "鈣",
			"钛": "鈦",
			"钜": "鉅",
			"钝": "鈍",
			"钞": "鈔",
			"钠": "鈉",
			"钡": "鋇",
			"钢": "鋼",
			"钣": "鈑",
			"钤": "鈐",
			"钥": "鑰",
			"钦": "欽",
			"钧": "鈞",
			"钨": "鎢",
			"钩": "鉤",
			"钪": "鈧",
			"钫": "鈁",
			"钬": "鈥",
			"钭": "鈄",
			"钮": "鈕",
			"钯": "鈀",
			"钰": "鈺",
			"钱": "錢",
			"钲": "鉦",
			"钳": "鉗",
			"钴": "鈷",
			"钶": "鈳",
			"钷": "鉕",
			"钹": "鈸",
			"钺": "鉞",
			"钼": "鉬",
			"钽": "鉭",
			"钾": "鉀",
			"钿": "鈿",
			"铀": "鈾",
			"铁": "鐵",
			"铂": "鉑",
			"铃": "鈴",
			"铄": "鑠",
			"铅": "鉛",
			"铆": "鉚",
			"铈": "鈰",
			"铉": "鉉",
			"铊": "鉈",
			"铋": "鉍",
			"铌": "鈮",
			"铍": "鈹",
			"铎": "鐸",
			"铏": "鉶",
			"铐": "銬",
			"铑": "銠",
			"铒": "鉺",
			"铓": "鋩",
			"铕": "銪",
			"铖": "鋮",
			"铗": "鋏",
			"铘": "鋣",
			"铙": "鐃",
			"铛": "鐺",
			"铜": "銅",
			"铝": "鋁",
			"铞": "銱",
			"铟": "銦",
			"铠": "鎧",
			"铡": "鍘",
			"铢": "銖",
			"铣": "銑",
			"铤": "鋌",
			"铥": "銩",
			"铧": "鏵",
			"铨": "銓",
			"铩": "鎩",
			"铪": "鉿",
			"铫": "銚",
			"铬": "鉻",
			"铭": "銘",
			"铮": "錚",
			"铯": "銫",
			"铰": "鉸",
			"铱": "銥",
			"铲": "鏟",
			"铳": "銃",
			"铴": "鐋",
			"铵": "銨",
			"银": "銀",
			"铷": "銣",
			"铸": "鑄",
			"铹": "鐒",
			"铺": "鋪",
			"铼": "錸",
			"铽": "鋱",
			"链": "鏈",
			"铿": "鏗",
			"销": "銷",
			"锁": "鎖",
			"锂": "鋰",
			"锃": "鋥",
			"锄": "鋤",
			"锅": "鍋",
			"锆": "鋯",
			"锇": "鋨",
			"锉": "銼",
			"锊": "鋝",
			"锋": "鋒",
			"锌": "鋅",
			"锍": "鋶",
			"锎": "鐦",
			"锐": "銳",
			"锑": "銻",
			"锒": "鋃",
			"锓": "鋟",
			"锔": "鋦",
			"锕": "錒",
			"锖": "錆",
			"锗": "鍺",
			"锘": "鍩",
			"错": "錯",
			"锚": "錨",
			"锛": "錛",
			"锜": "錡",
			"锝": "鍀",
			"锞": "錁",
			"锟": "錕",
			"锡": "錫",
			"锢": "錮",
			"锣": "鑼",
			"锤": "錘",
			"锥": "錐",
			"锦": "錦",
			"锧": "鑕",
			"锩": "錈",
			"锪": "鍃",
			"锫": "錇",
			"锬": "錟",
			"锭": "錠",
			"键": "鍵",
			"锯": "鋸",
			"锰": "錳",
			"锱": "錙",
			"锲": "鍥",
			"锴": "鍇",
			"锵": "鏘",
			"锶": "鍶",
			"锷": "鍔",
			"锸": "鍤",
			"锹": "鍬",
			"锺": "鍾",
			"锼": "鎪",
			"锾": "鍰",
			"锿": "鎄",
			"镀": "鍍",
			"镁": "鎂",
			"镂": "鏤",
			"镃": "鎡",
			"镄": "鐨",
			"镅": "鎇",
			"镆": "鏌",
			"镉": "鎘",
			"镊": "鑷",
			"镌": "鐫",
			"镍": "鎳",
			"镎": "鎿",
			"镏": "鎦",
			"镐": "鎬",
			"镑": "鎊",
			"镒": "鎰",
			"镓": "鎵",
			"镔": "鑌",
			"镕": "鎔",
			"镖": "鏢",
			"镗": "鏜",
			"镘": "鏝",
			"镙": "鏍",
			"镚": "鏰",
			"镛": "鏞",
			"镜": "鏡",
			"镝": "鏑",
			"镞": "鏃",
			"镟": "鏇",
			"镡": "鐔",
			"镣": "鐐",
			"镤": "鏷",
			"镥": "鑥",
			"镦": "鐓",
			"镧": "鑭",
			"镨": "鐠",
			"镩": "鑹",
			"镪": "鏹",
			"镫": "鐙",
			"镬": "鑊",
			"镭": "鐳",
			"镮": "鐶",
			"镯": "鐲",
			"镰": "鐮",
			"镱": "鐿",
			"镲": "鑔",
			"镳": "鑣",
			"镴": "鑞",
			"镶": "鑲",
			"长": "長",
			"门": "門",
			"闩": "閂",
			"闪": "閃",
			"闫": "閆",
			"闭": "閉",
			"问": "問",
			"闯": "闖",
			"闰": "閏",
			"闱": "闈",
			"闳": "閎",
			"间": "間",
			"闵": "閔",
			"闶": "閌",
			"闷": "悶",
			"闸": "閘",
			"闹": "鬧",
			"闺": "閨",
			"闻": "聞",
			"闼": "闥",
			"闽": "閩",
			"闾": "閭",
			"闿": "闓",
			"阀": "閥",
			"阁": "閣",
			"阂": "閡",
			"阃": "閫",
			"阄": "鬮",
			"阅": "閱",
			"阆": "閬",
			"阈": "閾",
			"阉": "閹",
			"阊": "閶",
			"阋": "鬩",
			"阌": "閿",
			"阍": "閽",
			"阎": "閻",
			"阏": "閼",
			"阐": "闡",
			"阑": "闌",
			"阒": "闃",
			"阔": "闊",
			"阕": "闋",
			"阖": "闔",
			"阗": "闐",
			"阙": "闕",
			"阚": "闞",
			"队": "隊",
			"阳": "陽",
			"阴": "陰",
			"阵": "陣",
			"阶": "階",
			"际": "際",
			"陆": "陸",
			"陇": "隴",
			"陈": "陳",
			"陉": "陘",
			"陕": "陝",
			"陧": "隉",
			"陨": "隕",
			"险": "險",
			"随": "隨",
			"隐": "隱",
			"隶": "隸",
			"隽": "雋",
			"难": "難",
			"雏": "雛",
			"雠": "讎",
			"雳": "靂",
			"雾": "霧",
			"霁": "霽",
			"霭": "靄",
			"靓": "靚",
			"静": "靜",
			"靥": "靨",
			"鞑": "韃",
			"鞯": "韉",
			"韦": "韋",
			"韧": "韌",
			"韨": "韍",
			"韩": "韓",
			"韪": "韙",
			"韫": "韞",
			"韬": "韜",
			"韵": "韻",
			"页": "頁",
			"顶": "頂",
			"顷": "頃",
			"顸": "頇",
			"项": "項",
			"顺": "順",
			"须": "須",
			"顼": "頊",
			"顽": "頑",
			"顾": "顧",
			"顿": "頓",
			"颀": "頎",
			"颁": "頒",
			"颂": "頌",
			"颃": "頏",
			"预": "預",
			"颅": "顱",
			"领": "領",
			"颇": "頗",
			"颈": "頸",
			"颉": "頡",
			"颊": "頰",
			"颋": "頲",
			"颌": "頜",
			"颍": "潁",
			"颏": "頦",
			"颐": "頤",
			"频": "頻",
			"颓": "頹",
			"颔": "頷",
			"颖": "穎",
			"颗": "顆",
			"题": "題",
			"颙": "顒",
			"颛": "顓",
			"颜": "顏",
			"额": "額",
			"颞": "顳",
			"颟": "顢",
			"颠": "顛",
			"颡": "顙",
			"颢": "顥",
			"颤": "顫",
			"颥": "顬",
			"颦": "顰",
			"颧": "顴",
			"风": "風",
			"飑": "颮",
			"飒": "颯",
			"飓": "颶",
			"飔": "颸",
			"飕": "颼",
			"飗": "飀",
			"飘": "飄",
			"飙": "飆",
			"飚": "飈",
			"飞": "飛",
			"飨": "饗",
			"餍": "饜",
			"饥": "飢",
			"饦": "飥",
			"饧": "餳",
			"饨": "飩",
			"饩": "餼",
			"饪": "飪",
			"饫": "飫",
			"饬": "飭",
			"饭": "飯",
			"饮": "飲",
			"饯": "餞",
			"饰": "飾",
			"饱": "飽",
			"饲": "飼",
			"饳": "飿",
			"饴": "飴",
			"饵": "餌",
			"饶": "饒",
			"饷": "餉",
			"饸": "餄",
			"饹": "餎",
			"饺": "餃",
			"饻": "餏",
			"饼": "餅",
			"饽": "餑",
			"饿": "餓",
			"馀": "餘",
			"馁": "餒",
			"馃": "餜",
			"馄": "餛",
			"馅": "餡",
			"馆": "館",
			"馇": "餷",
			"馈": "饋",
			"馉": "餶",
			"馊": "餿",
			"馋": "饞",
			"馍": "饃",
			"馎": "餺",
			"馏": "餾",
			"馐": "饈",
			"馑": "饉",
			"馒": "饅",
			"馓": "饊",
			"馔": "饌",
			"马": "馬",
			"驭": "馭",
			"驮": "馱",
			"驯": "馴",
			"驰": "馳",
			"驱": "驅",
			"驳": "駁",
			"驴": "驢",
			"驵": "駔",
			"驶": "駛",
			"驷": "駟",
			"驸": "駙",
			"驹": "駒",
			"驺": "騶",
			"驻": "駐",
			"驼": "駝",
			"驽": "駑",
			"驾": "駕",
			"驿": "驛",
			"骀": "駘",
			"骁": "驍",
			"骂": "罵",
			"骄": "驕",
			"骅": "驊",
			"骆": "駱",
			"骇": "駭",
			"骈": "駢",
			"骊": "驪",
			"骋": "騁",
			"验": "驗",
			"骎": "駸",
			"骏": "駿",
			"骐": "騏",
			"骑": "騎",
			"骒": "騍",
			"骓": "騅",
			"骖": "驂",
			"骗": "騙",
			"骘": "騭",
			"骚": "騷",
			"骛": "騖",
			"骜": "驁",
			"骝": "騮",
			"骞": "騫",
			"骟": "騸",
			"骠": "驃",
			"骡": "騾",
			"骢": "驄",
			"骣": "驏",
			"骤": "驟",
			"骥": "驥",
			"骧": "驤",
			"髅": "髏",
			"髋": "髖",
			"髌": "髕",
			"鬓": "鬢",
			"魇": "魘",
			"魉": "魎",
			"鱼": "魚",
			"鱽": "魛",
			"鱿": "魷",
			"鲁": "魯",
			"鲂": "魴",
			"鲅": "鮁",
			"鲆": "鮃",
			"鲈": "鱸",
			"鲊": "鮓",
			"鲋": "鮒",
			"鲍": "鮑",
			"鲎": "鱟",
			"鲏": "鮍",
			"鲐": "鮐",
			"鲑": "鮭",
			"鲒": "鮚",
			"鲔": "鮪",
			"鲕": "鮞",
			"鲖": "鮦",
			"鲗": "鰂",
			"鲙": "鱠",
			"鲚": "鱭",
			"鲛": "鮫",
			"鲜": "鮮",
			"鲝": "鮺",
			"鲞": "鯗",
			"鲟": "鱘",
			"鲠": "鯁",
			"鲡": "鱺",
			"鲢": "鰱",
			"鲣": "鰹",
			"鲤": "鯉",
			"鲥": "鰣",
			"鲦": "鰷",
			"鲧": "鯀",
			"鲨": "鯊",
			"鲩": "鯇",
			"鲫": "鯽",
			"鲭": "鯖",
			"鲮": "鯪",
			"鲰": "鯫",
			"鲱": "鯡",
			"鲲": "鯤",
			"鲳": "鯧",
			"鲴": "鯝",
			"鲵": "鯢",
			"鲷": "鯛",
			"鲸": "鯨",
			"鲺": "鯴",
			"鲻": "鯔",
			"鲼": "鱝",
			"鲽": "鰈",
			"鲿": "鱨",
			"鳃": "鰓",
			"鳄": "鱷",
			"鳅": "鰍",
			"鳆": "鰒",
			"鳇": "鰉",
			"鳊": "鯿",
			"鳋": "鰠",
			"鳌": "鰲",
			"鳍": "鰭",
			"鳎": "鰨",
			"鳏": "鰥",
			"鳐": "鰩",
			"鳑": "鰟",
			"鳒": "鰜",
			"鳓": "鰳",
			"鳔": "鰾",
			"鳕": "鱈",
			"鳖": "鱉",
			"鳗": "鰻",
			"鳘": "鰵",
			"鳙": "鱅",
			"鳛": "鰼",
			"鳜": "鱖",
			"鳝": "鱔",
			"鳞": "鱗",
			"鳟": "鱒",
			"鳢": "鱧",
			"鳣": "鱣",
			"鸟": "鳥",
			"鸠": "鳩",
			"鸢": "鳶",
			"鸣": "鳴",
			"鸥": "鷗",
			"鸦": "鴉",
			"鸧": "鶬",
			"鸨": "鴇",
			"鸩": "鴆",
			"鸪": "鴣",
			"鸫": "鶇",
			"鸬": "鸕",
			"鸭": "鴨",
			"鸮": "鴞",
			"鸯": "鴦",
			"鸰": "鴒",
			"鸱": "鴟",
			"鸲": "鴝",
			"鸳": "鴛",
			"鸵": "鴕",
			"鸶": "鷥",
			"鸷": "鷙",
			"鸸": "鴯",
			"鸹": "鴰",
			"鸺": "鵂",
			"鸻": "鴴",
			"鸼": "鵃",
			"鸽": "鴿",
			"鸾": "鸞",
			"鸿": "鴻",
			"鹁": "鵓",
			"鹂": "鸝",
			"鹃": "鵑",
			"鹄": "鵠",
			"鹅": "鵝",
			"鹆": "鵒",
			"鹈": "鵜",
			"鹉": "鵡",
			"鹊": "鵲",
			"鹋": "鶓",
			"鹌": "鵪",
			"鹎": "鵯",
			"鹏": "鵬",
			"鹐": "鵮",
			"鹑": "鶉",
			"鹒": "鶊",
			"鹕": "鶘",
			"鹖": "鶡",
			"鹗": "鶚",
			"鹘": "鶻",
			"鹙": "鶖",
			"鹚": "鶿",
			"鹛": "鶥",
			"鹜": "鶩",
			"鹞": "鷂",
			"鹡": "鶺",
			"鹣": "鶼",
			"鹤": "鶴",
			"鹥": "鷖",
			"鹦": "鸚",
			"鹧": "鷓",
			"鹨": "鷚",
			"鹩": "鷯",
			"鹪": "鷦",
			"鹫": "鷲",
			"鹬": "鷸",
			"鹭": "鷺",
			"鹯": "鸇",
			"鹰": "鷹",
			"鹱": "鸌",
			"鹳": "鸛",
			"鹾": "鹺",
			"麦": "麥",
			"麸": "麩",
			"黄": "黃",
			"黉": "黌",
			"黡": "黶",
			"黩": "黷",
			"黪": "黲",
			"黾": "黽",
			"鼋": "黿",
			"鼍": "鼉",
			"鼹": "鼴",
			"齐": "齊",
			"齑": "齏",
			"齿": "齒",
			"龀": "齔",
			"龃": "齟",
			"龄": "齡",
			"龅": "齙",
			"龆": "齠",
			"龇": "齜",
			"龈": "齦",
			"龉": "齬",
			"龊": "齪",
			"龋": "齲",
			"龌": "齷",
			"龙": "龍",
			"龚": "龔",
			"龛": "龕",
			"龟": "龜",
			"䴘": "鷈",
			"䴙": "鷿",
			"净": "淨",
			"啮": "齧",
			"埙": "壎",
			"滟": "灩",
			"绷": "繃",
			"艳": "豔",
			"莼": "蓴",
			"赝": "贗",
			"锈": "鏽",
			"镋": "钂",
			"鹇": "鷳",
			"勋": "勛",
			"岽": "崬",
			"棂": "櫺",
			"牦": "犛",
			"蘖": "櫱",
			"锨": "杴",
			"鞒": "屩",
			"馕": "饢",
			"闲": "閒",
			"荡": "蕩",
			"钟": "鐘",
			"仆": "僕",
			"佣": "傭",
			"凄": "淒",
			"着": "著",
			"里": "裡",
			"㑇": "㑳",
			"㘎": "㘚",
			"㤘": "㥮",
			"㧏": "掆",
			"㧐": "㩳",
			"㭎": "棡",
			"䅟": "穇",
			"䎬": "䎱",
			"䏝": "膞",
			"䓖": "藭",
			"䙌": "䙡",
			"䜩": "讌",
			"䦂": "䥇",
			"䦶": "䦛",
			"䦷": "䦟",
			"亘": "亙",
			"伲": "你",
			"倮": "裸",
			"凇": "淞",
			"凼": "幽",
			"剳": "劄",
			"勦": "剿",
			"卧": "臥",
			"厘": "釐",
			"叆": "靉",
			"叇": "靆",
			"哌": "呱",
			"啰": "囉",
			"嗬": "呵",
			"尜": "嘎",
			"峃": "嶨",
			"峯": "峰",
			"崐": "崑",
			"庄": "莊",
			"庼": "廎",
			"彚": "彙",
			"擀": "搟",
			"昵": "暱",
			"暸": "瞭",
			"杠": "槓",
			"榘": "矩",
			"檐": "簷",
			"氽": "汆",
			"沨": "渢",
			"沲": "沱",
			"浉": "溮",
			"浕": "濜",
			"涌": "湧",
			"涢": "溳",
			"溇": "漊",
			"炤": "照",
			"牀": "床",
			"珉": "玟",
			"珏": "玨",
			"琎": "璡",
			"琯": "管",
			"瓮": "甕",
			"畲": "畬",
			"痹": "痺",
			"癡": "痴",
			"硚": "礄",
			"礡": "礴",
			"稭": "秸",
			"糍": "餈",
			"紥": "紮",
			"翚": "翬",
			"腌": "醃",
			"腘": "膕",
			"膻": "羶",
			"萚": "蘀",
			"蟮": "蟺",
			"谉": "讅",
			"㑩": "儸",
			"㓥": "劏",
			"㔉": "劚",
			"㖊": "噚",
			"㟆": "㠏",
			"㧑": "撝",
			"㨫": "㩜",
			"㱩": "殰",
			"㱮": "殨",
			"㲿": "瀇",
			"㶉": "鸂",
			"㶶": "燶",
			"㶽": "煱",
			"㺍": "獱",
			"䅉": "稏",
			"䇲": "筴",
			"䌶": "䊷",
			"䌸": "縳",
			"䌹": "絅",
			"䌺": "䋙",
			"䌼": "綐",
			"䌽": "綵",
			"䌾": "䋻",
			"䍀": "繿",
			"䍁": "繸",
			"䓕": "薳",
			"䗖": "螮",
			"䙓": "襬",
			"䝙": "貙",
			"䞐": "賰",
			"䯄": "騧",
			"䯅": "䯀",
			"䲝": "䱽",
			"䲠": "鰆",
			"亸": "嚲",
			"伣": "俔",
			"俫": "倈",
			"厐": "龎",
			"吣": "唚",
			"啴": "嘽",
			"垱": "壋",
			"埯": "垵",
			"壸": "壼",
			"屃": "屓",
			"岙": "嶴",
			"崄": "嶮",
			"幞": "襆",
			"悮": "悞",
			"揾": "搵",
			"梼": "檮",
			"梾": "棶",
			"梿": "槤",
			"榅": "榲",
			"涂": "塗",
			"漤": "灠",
			"煴": "熅",
			"玚": "瑒",
			"珰": "璫",
			"疭": "瘲",
			"瘆": "瘮",
			"瞆": "瞶",
			"硁": "硜",
			"祃": "禡",
			"祢": "禰",
			"秾": "穠",
			"稆": "穭",
			"筜": "簹",
			"筼": "篔",
			"纟": "糹",
			"纻": "紵",
			"绤": "綌",
			"绬": "緓",
			"绹": "綯",
			"荙": "薘",
			"蔂": "虆",
			"蕰": "薀",
			"螀": "螿",
			"袆": "褘",
			"袯": "襏",
			"裈": "褌",
			"觍": "覥",
			"訚": "誾",
			"讠": "訁",
			"讬": "託",
			"讱": "訒",
			"诐": "詖",
			"诪": "譸",
			"贠": "貟",
			"赑": "贔",
			"赪": "赬",
			"跶": "躂",
			"轪": "軑",
			"辀": "輈",
			"辌": "輬",
			"辒": "轀",
			"酂": "酇",
			"酦": "醱",
			"钅": "釒",
			"钑": "鈒",
			"铇": "鉋",
			"铔": "錏",
			"铚": "銍",
			"铦": "銛",
			"铻": "鋙",
			"锠": "錩",
			"锳": "鍈",
			"锽": "鍠",
			"镈": "鎛",
			"镠": "鏐",
			"镵": "鑱",
			"闬": "閈",
			"阇": "闍",
			"阓": "闠",
			"阘": "闒",
			"阛": "闤",
			"霡": "霢",
			"颎": "熲",
			"颒": "頮",
			"颕": "頴",
			"飏": "颺",
			"飐": "颭",
			"飖": "颻",
			"饣": "飠",
			"饤": "飣",
			"饾": "餖",
			"馂": "餕",
			"馌": "饁",
			"驲": "馹",
			"骃": "駰",
			"骉": "驫",
			"骍": "騂",
			"骔": "騌",
			"骕": "驌",
			"骙": "騤",
			"骦": "驦",
			"鱾": "魢",
			"鲀": "魨",
			"鲃": "䰾",
			"鲄": "魺",
			"鲉": "鮋",
			"鲌": "鮊",
			"鲓": "鮳",
			"鲘": "鮜",
			"鲪": "鮶",
			"鲬": "鯒",
			"鲯": "鯕",
			"鲹": "鰺",
			"鲾": "鰏",
			"鳀": "鯷",
			"鳂": "鰃",
			"鳈": "鰁",
			"鳉": "鱂",
			"鳚": "䲁",
			"鳠": "鱯",
			"鳡": "鱤",
			"鸤": "鳲",
			"鸴": "鷽",
			"鹀": "鵐",
			"鹍": "鵾",
			"鹓": "鵷",
			"鹔": "鷫",
			"鹝": "鷊",
			"鹟": "鶲",
			"鹠": "鶹",
			"鹢": "鷁",
			"鹮": "䴉",
			"鹲": "鸏",
			"鹴": "鸘",
			"龁": "齕",
			"龂": "齗",
			"绣": "繡",
			"襕": "襴",
			"镢": "鐝",
			"䦆": "钁",
			"宁": "寧",
			"㝉": "宁",
			"苧": "薴",
			"苎": "苧",
			"硅": "矽",
			"㐷": "傌",
			"㐽": "偑",
			"㑈": "倲",
			"㑔": "㑯",
			"㚯": "㜄",
			"㛀": "媰",
			"㛣": "㜏",
			"㛤": "孋",
			"㧰": "擽",
			"㭏": "椲",
			"㭤": "樢",
			"㭴": "樫",
			"㳔": "濧",
			"㳕": "灡",
			"㳡": "濄",
			"㳽": "瀰",
			"㻅": "璯",
			"䀥": "䁻",
			"䂵": "碽",
			"䉤": "籔",
			"䌻": "䋚",
			"䌿": "䋹",
			"䥾": "䥱",
			"伡": "俥",
			"刾": "刺",
			"啯": "嘓",
			"垯": "墶",
			"媭": "嬃",
			"彟": "彠",
			"彨": "彲",
			"敩": "斆",
			"桪": "樳",
			"棱": "稜",
			"椢": "槶",
			"椫": "樿",
			"椮": "槮",
			"污": "汙",
			"澛": "瀂",
			"玙": "璵",
			"瑸": "璸",
			"硵": "磠",
			"秘": "祕",
			"篯": "籛",
			"翙": "翽",
			"荝": "萴",
			"詟": "讋",
			"颣": "纇",
			"鬶": "鬹",
			"鳤": "䲘",
			"麯": "麴",
			"潨": "潀",
			"睾": "睪",
			"钵": "缽",
			"参": "參",
			"叁": "叄",
			"么": "麼",
			"幺": "么",
			"为": "為",
			"产": "產",
			"体": "體",
			"㓰": "劃",
			"划": "劃",
			"国": "國",
			"奖": "獎",
			"并": "並",
			"开": "開",
			"术": "術",
			"规": "規",
			"视": "視",
			"赃": "贓",
			"酝": "醞",
			"钻": "鑽",
			"锻": "鍛",
			"镇": "鎮",
			"鸡": "雞",
			"药": "藥",
			"鉴": "鑑",
			"瘘": "瘺",
			"缐": "線",
			"线": "線",
			"裥": "襉",
			"钚": "鈽",
			"钸": "鈽",
			"腼": "靦",
			"䩄": "靦",
			"垅": "壟",
			"垄": "壟",
			"硷": "鹼",
			"碱": "鹼",
			"鳁": "鰮",
			"锏": "鐗",
			"缰": "韁",
			"棁": "梲",
			"捝": "挩",
			"姹": "奼",
			"咤": "吒",
			"颚": "顎",
			"腭": "齶",
			"鲶": "鯰",
			"鲇": "鮎",
			"䜧": "𧩣"
		}
	});
	MeihuaCC.addTable({
		name: '梅花通用詞彙',
		maxLen: 2,
		version: 140712,
		mappings: {
			"卷著": "捲著",
			"卷着": "捲著",
			"咽著": "嚥著",
			"咽着": "嚥著", //吃饭咽着怎么办
			"布著": "佈著", //天空滿佈著烏雲
			"布着": "佈著",
			"干著": "幹著",
			"干着": "幹著",
			"系著": "繫著",
			"系着": "繫著",
			"念著": "唸著",
			"念着": "唸著",
			"著干": "著幹",
			"着干": "著幹",
			"扎著": "紮著",
			"扎着": "紮著",
			"借著": "藉著",
			"借着": "藉著",
			"斗著": "鬥著",
			"斗着": "鬥著",
			"冲著": "衝著",
			"冲着": "衝著",
			"貴胄": "貴胄", //避免轉換胄->冑
			"華胄": "華胄",
			"遐胄": "遐胄",
			"胄子": "胄子",
			"胄裔": "胄裔",
			"裔胄": "裔胄",
			"乙丑": "乙丑", //天干地支
			"丁丑": "丁丑",
			"己丑": "己丑",
			"辛丑": "辛丑",
			"癸丑": "癸丑",
			"小丑": "小丑",
			"副丑": "副丑",
			"咸阳": "咸陽",
			"咸宜": "咸宜",
			"咸丰": "咸豐",
			"咸和": "咸和",
			"咸池": "咸池",
			"咸五": "咸五",
			"彭咸": "彭咸",
			"季咸": "季咸",
			"阮咸": "阮咸",
			"阿咸": "阿咸",
			"巫咸": "巫咸",
			"碧咸": "碧咸", //貝克漢
			"赞同": "贊同",
			"赞成": "贊成",
			"赞助": "贊助",
			"赞拜": "贊拜",
			"赞普": "贊普",
			"赞理": "贊理",
			"赞礼": "贊禮",
			"赞善": "贊善",
			"赞翼": "贊翼",
			"参赞": "參贊",
			"依赞": "依贊",
			"翊赞": "翊贊",
			"辅赞": "輔贊",
			"论赞": "論贊",
			"宣赞": "宣贊",
			"传赞": "傳贊",
			"像赞": "像贊", //為人物畫像所作的讚辭
			"象赞": "像贊", //錯誤的簡體
			"脏器": "臟器",
			"脏腑": "臟腑",
			"五脏": "五臟",
			"内脏": "內臟",
			"心脏": "心臟",
			"肝脏": "肝臟",
			"肺脏": "肺臟",
			"胃脏": "胃臟",
			"胰脏": "胰臟",
			"脺脏": "脺臟",
			"脾脏": "脾臟",
			"肾脏": "腎臟",
			"腑脏": "腑臟",
			"肠脏": "腸臟",
			"五岳": "五嶽",
			"中岳": "中嶽",
			"北岳": "北嶽",
			"南岳": "南嶽",
			"西岳": "西嶽",
			"东岳": "東嶽",
			"岳立": "嶽立", //淵渟嶽立
			"0余": "0餘",
			"0寸": "0吋",
			"1寸": "1吋",
			"2寸": "2吋",
			"3寸": "3吋",
			"4寸": "4吋",
			"5寸": "5吋",
			"6寸": "6吋",
			"7寸": "7吋",
			"8寸": "8吋",
			"9寸": "9吋",
			"○余": "○餘",
			"一伙": "一夥",
			"一只": "一隻",
			"一周": "一週",
			"一松": "一鬆",
			"一游": "一遊",
			"七只": "七隻",
			"丈余": "丈餘",
			"三只": "三隻",
			"三周": "三週",
			"上准": "上準", //某公司名
			"上周": "上週",
			"上折": "上摺",
			"上梁": "上樑",
			"下咽": "下嚥",
			"下梁": "下樑",
			"不余": "不餘",
			"不愈": "不癒",
			"不斗": "不鬥",
			"不系": "不繫",
			"不舍": "不捨",
			"不象": "不像",
			"不采": "不採",
			"主干": "主幹",
			"久斗": "久鬥",
			"之余": "之餘",
			"之准": "之準",
			"之游": "之遊",
			"九只": "九隻",
			"也象": "也像",
			"了望": "瞭望",
			"了然": "瞭然",
			"了解": "瞭解",
			"事干": "事幹", //找些充實的事幹
			"二只": "二隻",
			"互斗": "互鬥",
			"五只": "五隻",
			"五周": "五週",
			"五谷": "五穀",
			"些象": "些像", //長了些像水泡
			"交托": "交託",
			"交游": "交遊",
			"京沈": "京瀋",
			"人干": "人幹", //翻譯不是人幹的事
			"人游": "人遊",
			"人象": "人像",
			"什幺": "什麼",
			"仍斗": "仍鬥",
			"仍象": "仍像",
			"他干": "他幹",
			"他象": "他像",
			"付托": "付託",
			"仙游": "仙遊",
			"仙范": "仙範",
			"代征": "代徵",
			"令媛": "令嬡",
			"仿佛": "彷彿",
			"仿制": "仿製",
			"仿徉": "彷徉",
			"仿徨": "徬徨",
			"仿效": "倣傚",
			"伙伴": "夥伴",
			"伙同": "夥同",
			"伙夫": "伙伕",
			"伴伙": "伴夥",
			"伴游": "伴遊",
			"伶敏": "靈敏",
			"伸卷": "伸捲",
			"伺奉": "侍奉",
			"低迥": "低迴",
			"住扎": "住紮",
			"余丈": "餘丈",
			"余下": "餘下",
			"余事": "餘事",
			"余人": "餘人",
			"余位": "餘位",
			"余低": "餘低",
			"余僧": "餘僧",
			"余光": "餘光",
			"余刃": "餘刃",
			"余利": "餘利",
			"余剩": "餘剩",
			"余力": "餘力",
			"余勇": "餘勇",
			"余匪": "餘匪",
			"余匹": "餘匹",
			"余只": "餘隻",
			"余名": "餘名",
			"余味": "餘味",
			"余地": "餘地",
			"余天": "餘天",
			"余姚": "餘姚", //餘姚市
			"余威": "餘威",
			"余子": "餘子",
			"余字": "餘字",
			"余存": "餘存",
			"余孽": "餘孽",
			"余寸": "餘寸",
			"余尺": "餘尺",
			"余年": "餘年",
			"余座": "餘座",
			"余弦": "餘弦",
			"余怒": "餘怒",
			"余恨": "餘恨",
			"余息": "餘息",
			"余悸": "餘悸",
			"余情": "餘情",
			"余日": "餘日",
			"余暇": "餘暇",
			"余月": "餘月",
			"余杭": "餘杭",
			"余枝": "餘枝",
			"余桶": "餘桶",
			"余次": "餘次",
			"余款": "餘款",
			"余殃": "餘殃", //積惡餘殃
			"余毒": "餘毒",
			"余波": "餘波",
			"余漏": "餘漏",
			"余烈": "餘烈",
			"余物": "餘物",
			"余生": "餘生",
			"余缺": "餘缺",
			"余羹": "餘羹",
			"余者": "餘者",
			"余裕": "餘裕",
			"余路": "餘路",
			"余辜": "餘辜",
			"余震": "餘震",
			"余音": "餘音",
			"余香": "餘香",
			"佛象": "佛像",
			"作准": "作準",
			"作崇": "作祟",
			"你干": "你幹",
			"你象": "你像",
			"佳肴": "佳餚",
			"例范": "例範",
			"供制": "供製",
			"依准": "依準",
			"依托": "依託",
			"侵范": "侵範",
			"便松": "便鬆",
			"便象": "便像",
			"保准": "保準",
			"信托": "信託",
			"修浚": "修濬",
			"俱象": "俱像",
			"倍余": "倍餘",
			"倏松": "倏鬆",
			"倒卷": "倒捲",
			"倒象": "倒像",
			"借以": "藉以",
			"借口": "藉口",
			"借托": "借託",
			"借故": "藉故",
			"倦游": "倦遊",
			"停制": "停製",
			"停征": "停徵",
			"偶象": "偶像",
			"偷梁": "偷樑",
			"傍徨": "徬徨",
			"傒幸": "傒倖",
			"像征": "象徵",
			"儆效": "儆傚",
			"元凶": "元兇",
			"先奸": "先姦",
			"光念": "光唸", //光唸不做
			"克夫": "剋夫",
			"克扣": "剋扣",
			"克日": "剋日",
			"克星": "剋星",
			"克期": "剋期",
			"克死": "剋死",
			"克苦": "刻苦",
			"克薄": "剋薄",
			"免征": "免徵",
			"兜卷": "兜捲",
			"入伙": "入夥",
			"全象": "全像",
			"八只": "八隻",
			"八周": "八週",
			"公余": "公餘", //辦公以外的時間
			"公布": "公佈",
			"公干": "公幹",
			"六只": "六隻",
			"六周": "六週",
			"六欲": "六慾",
			"共制": "共製",
			"共御": "共禦",
			"共斗": "共鬥",
			"其余": "其餘",
			"典范": "典範",
			"再制": "再製",
			"再卷": "再捲", //錢江潮再捲人兩名
			"再干": "再幹",
			"再斗": "再鬥",
			"再松": "再鬆",
			"再象": "再像",
			"冗余": "冗餘",
			"冬台": "冬颱",
			"冶游": "冶遊",
			"准些": "準些",
			"准准": "準準", //彩金機率看準準
			"准到": "準到",
			"准定": "準定", //一日暢玩飆水、準定讓你High翻天
			"准度": "準度",
			"准得": "準得",
			"准星": "準星",
			"准是": "準是",
			"准有": "準有", //技術分析有時準有時不準
			"死准": "死準",
			"准直": "準直", //準直器、準直透鏡
			"准知": "準知", //你父親準知我在你眼前蒙恩
			"准能": "準能",
			"准被": "準被", //發音不準被質疑
			"准要": "準要",
			"凝干": "凝乾",
			"凶嫌": "兇嫌",
			"凶徒": "兇徒",
			"凶案": "兇案",
			"凶犯": "兇犯",
			"凶猛": "兇猛",
			"出游": "出遊",
			"函容": "涵容",
			"函意": "涵意",
			"函洞": "涵洞",
			"函濡": "涵濡",
			"函煦": "涵煦",
			"函管": "涵管",
			"函蓄": "涵蓄",
			"散伙": "散夥",
			"分伙": "分夥",
			"分布": "分佈",
			"分象": "分像", //三分像人
			"分量": "份量",
			"刑余": "刑餘",
			"初愈": "初癒",
			"初斗": "初鬥",
			"刨刀": "鉋刀",
			"刨削": "鉋削",
			"刨子": "鉋子",
			"刨花": "鉋花",
			"利欲": "利慾",
			"刮倒": "颳倒",
			"刮胡": "刮鬍",
			"刮起": "颳起",
			"制丹": "製丹",
			"制件": "製件",
			"制作": "製作",
			"制做": "製做",
			"制冰": "製冰",
			"制冷": "製冷",
			"制出": "製出",
			"制品": "製品",
			"制器": "製器",
			"制成": "製成",
			"制播": "製播",
			"制材": "製材",
			"制毒": "製毒",
			"制法": "製法",
			"制片": "製片",
			"制版": "製版",
			"制程": "製程",
			"制糖": "製糖",
			"制衣": "製衣",
			"制表": "製表",
			"制造": "製造",
			"制醋": "製醋",
			"制面": "製麵",
			"制革": "製革",
			"制鞋": "製鞋",
			"刺准": "刺準",
			"刻象": "刻像",
			"剉冰": "銼冰",
			"剉刀": "銼刀",
			"剉工": "銼工",
			"剉平": "銼平",
			"土剉": "土銼",
			"扁剉": "扁銼",
			"板剉": "板銼",
			"剩肴": "剩餚",
			"剪彩": "剪綵",
			"剪除": "翦除",
			"割舍": "割捨",
			"力干": "力幹",
			"力拼": "力拚",
			"力斗": "力鬥",
			"劫余": "劫餘",
			"劾系": "劾繫", //彈劾過失並拘禁
			"勇斗": "勇鬥",
			"勤朴": "勤樸",
			"勾干": "勾幹", //n.壞事 v.賄賂
			"包准": "包準",
			"包函": "包涵",
			"包卷": "包捲",
			"包扎": "包紮",
			"包谷": "包穀", //玉蜀黍的別名
			"北回": "北迴",
			"北游": "北遊",
			"匪干": "匪幹",
			"十余": "十餘",
			"十只": "十隻",
			"十周": "十週",
			"千余": "千餘",
			"千只": "千隻",
			"卅余": "卅餘",
			"升官": "陞官",
			"升平": "昇平",
			"升汞": "昇汞", //學名為氯化汞
			"升泰": "昇泰",
			"半只": "半隻",
			"半干": "半乾",
			"南回": "南迴",
			"南游": "南遊",
			"博斗": "博鬥",
			"印制": "印製",
			"卷上": "捲上",
			"卷了": "捲了",
			"卷住": "捲住",
			"卷入": "捲入",
			"卷出": "捲出",
			"卷到": "捲到",
			"卷去": "捲去",
			"卷向": "捲向",
			"卷回": "捲回",
			"卷土": "捲土", //捲土重來
			"卷往": "捲往",
			"卷得": "捲得",
			"卷成": "捲成",
			"卷拂": "捲拂",
			"卷款": "捲款",
			"卷毛": "捲毛",
			"卷浪": "捲浪",
			"卷碎": "捲碎",
			"卷至": "捲至",
			"卷舌": "捲舌",
			"卷袖": "捲袖",
			"卷裹": "捲裹",
			"卷走": "捲走",
			"卷起": "捲起",
			"卷身": "捲身",
			"卷逃": "捲逃",
			"反卷": "反捲",
			"去卷": "去捲",
			"去干": "去幹",
			"又干": "又幹",
			"又斗": "又鬥",
			"又松": "又鬆",
			"又象": "又像",
			"取余": "取餘",
			"取舍": "取捨",
			"受托": "受託",
			"受雇": "受僱",
			"口干": "口乾",
			"古朴": "古樸",
			"另辟": "另闢",
			"只余": "只餘",
			"只兔": "隻兔",
			"只字": "隻字",
			"只影": "隻影",
			"只手": "隻手",
			"只牛": "隻牛",
			"只犬": "隻犬",
			"只狗": "隻狗",
			"只狼": "隻狼",
			"只猴": "隻猴",
			"只眼": "隻眼",
			"只羊": "隻羊",
			"只虎": "隻虎",
			"只蛋": "隻蛋",
			"只身": "隻身",
			"只鬼": "隻鬼",
			"只鹿": "隻鹿",
			"可象": "可像",
			"可采": "可採",
			"台凳": "檯凳",
			"台制": "台製",
			"台子": "檯子",
			"台布": "檯布",
			"台干": "台幹",
			"台面": "檯面",
			"吃准": "吃準",
			"吃干": "吃乾", //吃乾抹淨
			"吃茶": "喫茶",
			"吃面": "吃麵",
			"合伙": "合夥",
			"合府": "闔府",
			"合斗": "合鬥",
			"合眼": "闔眼",
			"合第": "闔第",
			"吊信": "弔信",
			"吊古": "弔古", //感念往昔的人事
			"吊唁": "弔唁",
			"吊奠": "弔奠",
			"吊孝": "弔孝",
			"吊客": "弔客",
			"吊影": "弔影",
			"吊念": "弔念",
			"吊慰": "弔慰",
			"吊文": "弔文",
			"吊民": "弔民", //弔民伐罪
			"吊祭": "弔祭",
			"同伙": "同夥",
			"同游": "同遊",
			"吐干": "吐乾", //口水吐乾
			"向准": "向準", //臺鐵邁向準高鐵
			"向午": "晌午",
			"向往": "嚮往",
			"吞咽": "吞嚥",
			"吟游": "吟遊",
			"吧台": "吧檯",
			"吮干": "吮乾", //輕柔地吮乾她的淚水
			"吵斗": "吵鬥",
			"吸干": "吸乾",
			"吹卷": "吹捲",
			"吹干": "吹乾",
			"吹胡": "吹鬍",
			"周一": "週一",
			"周三": "週三",
			"周二": "週二",
			"周五": "週五",
			"周休": "週休",
			"周六": "週六",
			"周刊": "週刊",
			"周四": "週四",
			"周年": "週年",
			"周息": "週息",
			"周日": "週日",
			"周期": "週期",
			"周末": "週末",
			"周波": "週波",
			"周游": "周遊",
			"周薪": "週薪",
			"周遭": "週遭",
			"呷干": "呷乾",
			"呼卷": "呼捲", //一陣呼卷的風塵
			"咀咽": "咀嚥",
			"咎征": "咎徵",
			"咨商": "諮商",
			"咽下": "嚥下",
			"咽了": "嚥了",
			"咽住": "嚥住",
			"咽入": "嚥入",
			"咽唾": "嚥唾",
			"咽嚼": "嚥嚼",
			"咽回": "嚥回", //把淚嚥回肚中
			"咽食": "嚥食", //嚥食困難
			"哀吊": "哀弔",
			"哪象": "哪像",
			"唁吊": "唁弔",
			"唱游": "唱遊",
			"喂它": "餵牠",
			"喂羊": "餵羊",
			"善斗": "善鬥",
			"喉干": "喉乾", //喉乾舌燥
			"喝干": "喝乾",
			"喧染": "渲染",
			"嘴干": "嘴乾",
			"嘹亮": "嘹喨",
			"四只": "四隻",
			"四舍": "四捨", //四捨五入
			"回卷": "回捲",
			"回圈": "迴圈",
			"回廊": "迴廊",
			"回旋": "迴旋",
			"回游": "迴游",
			"回翔": "迴翔",
			"回避": "迴避",
			"困乏": "睏乏",
			"困倦": "睏倦",
			"困意": "睏意",
			"圈梁": "圈樑", //建築四面牆壁內部的樑
			"土制": "土製", //土製炸彈
			"地卷": "地捲", //如何正確地捲電線
			"地志": "地誌",
			"均沾": "均霑",
			"坤范": "坤範", //婦女的模範
			"垂范": "垂範", //垂範百世
			"型范": "型範",
			"基准": "基準",
			"基干": "基幹",
			"塑象": "塑像",
			"墓志": "墓誌",
			"增辟": "增闢",
			"外制": "外製", //外製外包
			"多余": "多餘",
			"夜游": "夜遊",
			"大伙": "大夥",
			"大只": "大隻",
			"大斗": "大鬥",
			"天佑": "天祐",
			"太准": "太準",
			"太干": "太乾",
			"太松": "太鬆",
			"太象": "太像",
			"夫奸": "夫姦",
			"夫役": "伕役",
			"失准": "失準",
			"奇准": "奇準",
			"奏折": "奏摺",
			"奸凶": "姦凶", //攘除姦凶
			"奸夫": "姦夫",
			"奸情": "姦情",
			"奸拐": "姦拐", //花有憐姦拐紅顏
			"奸污": "姦污",
			"奸淫": "姦淫",
			"奸通": "姦通",
			"她干": "她幹",
			"她象": "她像",
			"好准": "好準",
			"好干": "好幹",
			"好斗": "好鬥",
			"好象": "好像",
			"委托": "委託",
			"姜是": "薑是",
			"姜末": "薑末",
			"姜汁": "薑汁",
			"姜油": "薑油",
			"姜片": "薑片",
			"姜糖": "薑糖",
			"姜花": "薑花",
			"姜茶": "薑茶",
			"姜醋": "薑醋",
			"娉亭": "娉婷",
			"婀挪": "婀娜",
			"嫌凶": "嫌兇",
			"嬉游": "嬉遊",
			"嬖幸": "嬖倖",
			"嬴余": "嬴餘",
			"子姜": "子薑",
			"存折": "存摺",
			"它象": "它像",
			"守御": "守禦",
			"宏基": "宏碁",
			"定制": "定製",
			"定干": "定幹",
			"客游": "客遊",
			"宣布": "宣佈",
			"宦游": "宦遊",
			"宴游": "宴遊",
			"家伙": "傢伙",
			"家俱": "傢俱",
			"家私": "傢俬",
			"宸游": "宸遊",
			"容范": "容範", //容貌儀範
			"寄托": "寄託",
			"密布": "密佈",
			"密折": "密摺",
			"密致": "密緻",
			"富余": "富餘",
			"寒喧": "寒暄",
			"寒栗": "寒慄",
			"寡欲": "寡慾",
			"寸余": "寸餘",
			"射准": "射準", //投球射準競賽
			"射雕": "射鵰",
			"尊范": "尊範",
			"小伙": "小夥",
			"少干": "少幹",
			"尚余": "尚餘",
			"尤如": "猶如",
			"尤疑": "猶疑",
			"尤豫": "猶豫",
			"就范": "就範",
			"就象": "就像",
			"尺余": "尺餘",
			"局促": "侷促",
			"局蹙": "跼蹙",
			"局躅": "跼躅",
			"局限": "侷限",
			"屋梁": "屋樑",
			"屯扎": "屯紮",
			"峰回": "峰迴",
			"巡回": "巡迴",
			"巡游": "巡遊",
			"工余": "工餘",
			"巧干": "巧幹",
			"巨制": "巨製",
			"己干": "己幹",
			"已余": "已餘",
			"已愈": "已癒",
			"已松": "已鬆",
			"已象": "已像",
			"布伏": "佈伏",
			"布署": "佈署",
			"布兵": "佈兵",
			"布告": "佈告",
			"布局": "佈局",
			"布施": "佈施",
			"布景": "佈景",
			"布置": "佈置",
			"布谷": "布穀",
			"布防": "佈防",
			"布雷": "佈雷",
			"席卷": "席捲",
			"席子": "蓆子",
			"常干": "常幹",
			"快干": "快乾",
			"干一": "幹一",
			"干上": "幹上",
			"干不": "幹不",
			"干了": "幹了",
			"干事": "幹事",
			"干井": "乾井",
			"干些": "幹些",
			"干什": "幹什",
			"干他": "幹他",
			"干件": "幹件",
			"干份": "幹份", //幹份兼職教英語
			"干似": "乾似", //臉乾似沙漠
			"干你": "幹你",
			"干假": "幹假", //市長幹假的？
			"干冰": "乾冰",
			"干冷": "乾冷",
			"干出": "幹出",
			"干到": "幹到",
			"干化": "乾化", //後梁太祖朱全忠的年號
			"干吏": "幹吏", //能幹練達的官吏
			"干咳": "乾咳",
			"干咽": "乾嚥",
			"干品": "乾品", //冬蟲夏草乾品
			"干哥": "乾哥",
			"干哪": "幹哪",
			"干啥": "幹啥",
			"干嘛": "幹嘛",
			"干嚎": "乾嚎",
			"干土": "乾土",
			"干坤": "乾坤",
			"干她": "幹她",
			"干好": "幹好", //幹好本職工作
			"干妹": "乾妹",
			"干姊": "乾姊",
			"干姐": "乾姐",
			"干姜": "乾薑",
			"干娘": "乾娘",
			"干季": "乾季",
			"干完": "幹完",
			"干巴": "乾巴",
			"干布": "乾布",
			"干干": "乾乾", //君子終日乾乾
			"干式": "乾式",
			"干弟": "乾弟",
			"干得": "幹得",
			"干性": "乾性",
			"干成": "幹成",
			"干我": "幹我",
			"干才": "幹才",
			"干掉": "幹掉",
			"干探": "幹探",
			"干料": "乾料",
			"干旱": "乾旱",
			"干材": "幹材",
			"干杯": "乾杯",
			"干果": "乾果",
			"干枝": "乾枝",
			"干枯": "乾枯",
			"干架": "幹架",
			"干柴": "乾柴",
			"干校": "幹校", //幹部學校
			"干梅": "乾梅",
			"干此": "幹此",
			"干沙": "乾沙",
			"干泥": "乾泥",
			"干洗": "乾洗",
			"干活": "幹活",
			"干流": "幹流", //河川下游內、對比於支流的主要河段
			"干涸": "乾涸",
			"干渴": "乾渴",
			"干焦": "乾焦",
			"干熬": "乾熬",
			"干燥": "乾燥",
			"干爸": "乾爸",
			"干爹": "乾爹",
			"干爽": "乾爽",
			"干球": "乾球", //乾球溫度計
			"干甚": "幹甚",
			"干疤": "乾疤", //水痘乾疤後多久掉？
			"干瘦": "乾瘦",
			"干眼": "乾眼",
			"干瞪": "乾瞪",
			"干硬": "乾硬", //寶寶大便乾硬怎麼辦？
			"干窘": "乾窘",
			"干笑": "乾笑",
			"干等": "乾等",
			"干粉": "乾粉",
			"干系": "干係",
			"干耗": "乾耗",
			"干肉": "乾肉",
			"干股": "乾股",
			"干脆": "乾脆",
			"干花": "乾花",
			"干草": "乾草",
			"干菜": "乾菜",
			"干薪": "乾薪",
			"干衣": "乾衣",
			"干裂": "乾裂",
			"干警": "幹警",
			"干起": "幹起",
			"干路": "幹路",
			"干透": "乾透",
			"干道": "幹道",
			"干那": "幹那",
			"干部": "幹部",
			"干酪": "乾酪",
			"干醋": "乾醋",
			"干隆": "乾隆",
			"干面": "乾麵",
			"平准": "平準", //平準基金
			"年余": "年餘",
			"幸免": "倖免",
			"幸存": "倖存",
			"幸幸": "倖幸",
			"幽郁": "幽鬱",
			"床席": "床蓆",
			"床第": "床笫",
			"店伙": "店夥",
			"座象": "座像", //董座像「蟹老闆」
			"康愈": "康癒",
			"廿余": "廿餘",
			"弄干": "弄乾",
			"弄松": "弄鬆",
			"弦歌": "絃歌",
			"弦琴": "絃琴",
			"弦管": "絃管",
			"弦索": "絃索",
			"弱干": "弱幹", //強枝弱幹
			"彩牌": "綵牌",
			"彩球": "綵球",
			"彩衣": "綵衣",
			"彩裳": "綵裳",
			"影象": "影像",
			"彷佛": "彷彿",
			"彷徨": "徬徨",
			"征了": "徵了",
			"征人": "徵人",
			"征信": "徵信",
			"征候": "徵候",
			"征兆": "徵兆",
			"征入": "徵入",
			"征兵": "徵兵",
			"征募": "徵募",
			"西征": "西征", //預防"西征去"過度轉換為"西徵去"
			"东征": "東征",
			"南征": "南征",
			"北征": "北征",
			"长征": "長征",
			"出征": "出征",
			"亲征": "親征",
			"征去": "徵去", //诚征去丽江合伙开客栈
			"征友": "徵友",
			"征取": "徵取", //設計方案徵取社會意見
			"征召": "徵召",
			"征地": "徵地",
			"征婚": "徵婚",
			"征引": "徵引",
			"征得": "徵得",
			"征收": "徵收",
			"征文": "徵文",
			"征求": "徵求",
			"征片": "徵片",
			"征用": "徵用",
			"征稿": "徵稿",
			"征管": "徵管", //稅收徵管法
			"征聘": "徵聘",
			"征象": "徵象", //生命徵象
			"征逐": "徵逐", //酒食徵逐
			"征集": "徵集",
			"很准": "很準",
			"很干": "很乾",
			"很松": "很鬆",
			"很象": "很像",
			"徒托": "徒託", //徒託空言
			"得准": "得準", //打得準
			"得象": "得像", //哭得像小孩
			"御侮": "禦侮",
			"御制": "御製",
			"御寇": "禦寇",
			"御寒": "禦寒",
			"徨恐": "惶恐",
			"微松": "微鬆",
			"心系": "心繫",
			"必游": "必遊",
			"志哀": "誌哀",
			"志喜": "誌喜",
			"快卷": "快捲",
			"念佛": "唸佛",
			"念作": "唸作",
			"念到": "唸到",
			"念咒": "唸咒",
			"念好": "唸好",
			"念完": "唸完",
			"念得": "唸得",
			"念念": "唸唸",
			"念成": "唸成",
			"念法": "唸法",
			"忽松": "忽鬆",
			"怎幺": "怎麼",
			"怖栗": "怖慄",
			"急卷": "急捲",
			"急征": "急徵",
			"性征": "性徵",
			"性欲": "性慾",
			"恂栗": "恂慄", //畏懼害怕
			"恰象": "恰像", //恰像颱風掃寰宇
			"悒郁": "悒鬱",
			"悚栗": "悚慄",
			"悠游": "悠遊",
			"悲戚": "悲慼",
			"悲泯": "悲憫",
			"悲郁": "悲鬱",
			"悸栗": "悸慄",
			"情欲": "情慾",
			"惕栗": "惕慄",
			"想干": "想幹",
			"想斗": "想鬥",
			"想象": "想像",
			"惴栗": "惴慄",
			"惶栗": "惶慄",
			"惺松": "惺忪",
			"愁郁": "愁鬱",
			"愈卷": "愈捲", //風暴愈捲愈大
			"越卷": "越捲",
			"愈合": "癒合",
			"愈象": "愈像",
			"意函": "意涵",
			"愚暗": "愚闇",
			"慢咽": "慢嚥",
			"慰借": "慰藉",
			"懈松": "懈鬆",
			"懊郁": "懊鬱",
			"郁懊": "鬱懊",
			"懰栗": "懰慄", //憂傷悲愴
			"懿范": "懿範",
			"我干": "我幹",
			"我象": "我像",
			"或松": "或鬆", //房價受控調控下半年或鬆
			"或采": "或採", //或採鏤空發光設計
			"戚嗟": "慼嗟",
			"戚然": "慼然",
			"戴表": "戴錶",
			"所余": "所餘",
			"所干": "所幹", //今天所幹的工作
			"所系": "所繫", //國運所繫、政之所行
			"改采": "改採",
			"手松": "手鬆",
			"手表": "手錶",
			"手采": "手採", //頂級手採白毫綠茶
			"才干": "才幹",
			"才象": "才像",
			"扎上": "紮上", //紮上繃帶
			"扎住": "紮住", //軍士紮住在此
			"扎囮": "紮囮",
			"扎寨": "紮寨",
			"扎彩": "紮彩", //紮彩是一種民間工藝品
			"扎成": "紮成", //紮成一個馬尾
			"扎裹": "紮裹",
			"扎起": "紮起", //紮起馬尾
			"打制": "打製",
			"打斗": "打鬥",
			"打谷": "打穀",
			"酒托": "酒托", //預防"遇到酒托了"過度轉換為"遇到酒託了"
			"当托": "當托",
			"托了": "託了",
			"托事": "託事",
			"托交": "託交",
			"托人": "託人",
			"托付": "託付",
			"托古": "託古", //託古改制
			"托名": "託名",
			"托咎": "託咎",
			"托大": "託大",
			"托孤": "託孤",
			"托故": "託故",
			"托疾": "託疾",
			"托病": "託病",
			"托管": "託管",
			"托言": "託言",
			"托身": "託身",
			"扛丸": "摃丸",
			"扣准": "扣準", //佈局扣準改革與轉型
			"扮象": "扮像",
			"扶余": "扶餘", //扶餘國
			"批斗": "批鬥",
			"批注": "批註",
			"找准": "找準", //找準生物鐘同步即健康
			"承制": "承製",
			"把姜": "把薑", //如何把薑快速去皮
			"抑郁": "抑鬱",
			"抓准": "抓準",
			"抓奸": "抓姦",
			"抓斗": "抓鬥",
			"投准": "投準",
			"抗御": "抗禦",
			"抗斗": "抗鬥",
			"折子": "摺子",
			"折扇": "摺扇",
			"折痕": "摺痕",
			"折裙": "摺裙",
			"抬扛": "抬摃",
			"抵御": "抵禦",
			"抹台": "抹檯",
			"抹干": "抹乾",
			"抽干": "抽乾",
			"拆伙": "拆夥",
			"拆斗": "拆鬥", //已拆鬥三百餘招
			"拉面": "拉麵",
			"拌面": "拌麵"
		}
	});

	var listenElmt = document.getElementById('appcontent');
	if (listenElmt) {
		listenElmt.addEventListener('DOMContentLoaded', MeihuaCC.onPageLoad);
	}
	window.MeihuaCC = MeihuaCC;
})(window);
